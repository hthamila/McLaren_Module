...
--select COUNT(*) from pce_qe16_prd..tmp_mort; --227364  --233208
...
round(max(tmi.mort_oe_rt),2) as mort_oe_rt,
(case 
...
round(max(tlu.lab_utlz),2) as lab_utlz,
...
group by 1,2,3
 ...
 select distinct rpt_dt from stg_clncl_outc_scor_fct; 
 
  select * from stg_clncl_outc_scor_fct WHERE rpt_dt = '2020-04-01';
  select * from pce_qe16_prd..stg_tmp_mrtly_ind WHERE rpt_dt = '2020-04-01';
 
 select fcy_nm , rpt_dt,max(ROUND(tzef.zero_events_rt,2)) as y,
 (case 
when  max(ROUND(tzef.zero_events_rt,2)) < 0.75 then 0
when  max(ROUND(tzef.zero_events_rt,2)) >= 0.75 and max(ROUND(tzef.zero_events_rt,2)) < 0.82 then 50
when  max(ROUND(tzef.zero_events_rt,2)) >= 0.82 and max(ROUND(tzef.zero_events_rt,2)) < 0.86 then 75
...
when  max(ROUND(tzef.zero_events_rt,2)) >=0.95 then 150
 ...
 from stg_tmp_Zero_events_Fct tzef
 WHERE tzef.fcy_nm ='McLaren Greater Lansing'
 GROUP BY 1,2;
