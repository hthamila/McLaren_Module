...
when p.company_id ='Thumb' then '823599'  --FY23 change
when p.company_id ='Caro' then 'MI2194'  --FY23 change
...
when 'Thumb' then '823599'  --FY23 change
when 'Caro' then 'MI2194'  --FY23 change
else null end as fcy_num
,p.patient_id
...
when 'Macomb' then 'MI2048' when 'Northern' then '637619' when 'Oakland' then 'MI2055' when 'Port Huron' then '600816' when 'St. Lukes' then 'AB9813' when 'Thumb' then '823599'
when 'Caro' then 'MI2194' else null end) = s.fcy_num
...
,max(case when ef.ms_drg_icd10 in ('870','871','872') then 1 else 0 end ) as sepsis_icd_ind -- FY23 change
...
--	, case when (pds.updateid = 'Incarcerated') then 0 else 1 end as crnr_lnsg_prsnr_excl_ind
	...
	,tqa.COPD_ind
	--,tqa.sepsis_icd_ind
	,tqa.sepsis_icd_ind -- FY23 change
...
left join pce_qe16_pressganey_prd_zoom..prgny_ptnt_exrnc_pct_dim ppep on tmppm.service = ppep.svc_cd and tmppm.ptnt_exrnc_pct = ppep.scor and --year(tmppm.rprt_dt)=ppep.reporting_year)
CASE WHEN DATE_PART('MONTH',rprt_dt) > 9 THEN YEAR(tmppm.rprt_dt)+1= ppep.reporting_year
ELSE YEAR(tmppm.rprt_dt) = ppep.reporting_year END); --FY23 Change
...
when hsptl_rgon = 'Thumb' then '823599' --FY23 change
when hsptl_rgon = 'Caro' then 'MI2194' --FY23 change 
else null end as fcy_num
...
select 'stg_tmp_onc_ptnt_exrnc_pct_msr_fct';
DROP TABLE stg_tmp_onc_ptnt_exrnc_pct_msr_fct IF EXISTS;
create  table  stg_tmp_onc_ptnt_exrnc_pct_msr_fct as 
(select tspe.service,
tspe.client_id,
tspe.FCY_NM,
...
--FY23 Change
SELECT 'tmp_onc_ptnt_exrnc_subsidaries';
DROP TABLE tmp_onc_ptnt_exrnc_subsidaries IF EXISTS;
CREATE TABLE tmp_onc_ptnt_exrnc_subsidaries AS
(
SELECT 
    survey_id,
    varname,
    value,
    CASE 
        WHEN (UPPER(VALUE) IN ('FNTRO ', 'FNTROOW ', 'FNTSO', 'FNTGYNO', 'FNTMO', 'FNTIC', 'FNTROLP', 'FNTRROTN') AND UPPER(varname) = 'UNIT') THEN 'Flint'
        WHEN (UPPER(VALUE) IN ('MNMICG', 'MNMIC', 'MNMRO', 'MNMMOG') AND UPPER(varname) = 'UNIT') THEN 'Northern'
        WHEN (UPPER(VALUE) IN ('LANRO', 'LANMMP', 'LANIC', 'LANMDC', 'LANMO', 'LANMMPIC') AND UPPER(varname) = 'UNIT') THEN 'Lansing'
        WHEN (UPPER(VALUE) IN ('LN93 MMP MO', 'LN93 MMP IC') AND UPPER(varname) = 'ITUNIT') THEN 'Lansing'
        WHEN (UPPER(VALUE) IN ('BAYRO', 'BAYIC', 'BAYMO', 'BAYWBMO') AND UPPER(varname) = 'UNIT') THEN 'Bay'
        WHEN (UPPER(VALUE) IN ('CNTMO', 'CNTRO', 'CNTIC') AND UPPER(varname) = 'UNIT') THEN 'Central'
        WHEN (UPPER(VALUE) IN ('OKMOCLK', 'OKICCLK') AND UPPER(varname) = 'UNIT') THEN 'Oakland'
        WHEN (UPPER(VALUE) IN ('MACROCLK', 'MACRO', 'MACMO', 'ORTHMAC', 'MACIC') AND UPPER(varname) = 'UNIT') THEN 'Macomb'
        WHEN (UPPER(VALUE) IN ('MC17 KCI CLK RO', 'MC14 KCI MAC RO', 'OK02 KCI MAC OO') AND UPPER(varname) = 'ITUNIT') THEN 'Macomb'
        WHEN (UPPER(VALUE) IN ('RADONCKCI') AND UPPER(varname) = 'ITLOCATI') THEN 'Port Huron'
        WHEN (UPPER(VALUE) IN ('RADONPH') AND UPPER(varname) = 'UNIT') THEN 'Port Huron'
        WHEN (UPPER(VALUE) IN ('LAPIC', 'LAPMO') AND UPPER(varname) = 'UNIT') THEN 'Lapeer'
        ELSE NULL
    END AS optnt_onc_subsidary
FROM
    pce_qe16_pressganey_prd_zoom.prmradmp.cv_survey_demographics
WHERE 
    optnt_onc_subsidary IS NOT NULL
)
;

--FY23 Change
-- Outpatient Oncology Likelihood to Recommend
SELECT 'tmp_sub_op_onc_ptnt_exrnc_msr_fct';
DROP TABLE tmp_sub_op_onc_ptnt_exrnc_msr_fct IF EXISTS;
CREATE TABLE tmp_sub_op_onc_ptnt_exrnc_msr_fct AS
(
SELECT 
DISTINCT 
    tsf.service, 
    pes.optnt_onc_subsidary AS fcy_nm,
    tm.dschrg_dt AS rprt_dt,
    SUM(
       (
       CASE
           WHEN tsf.survey_type = 'CAHPS' AND tsf.top_box_answer IS NOT NULL AND tsf.resp_val IN ( 'Yes' , 'Always','Strongly agree','Definitely yes', 'Yes, definitely', 'Yes', '9' ,'10-Best possible', '10-Best facility possible', '10-Best provider') THEN 1
           WHEN tsf.survey_type = 'CAHPS' AND tsf.top_box_answer IS NOT NULL THEN 0
           WHEN tsf.survey_type = 'OTHER' AND tsf.resp_val = '5' THEN 1
           WHEN tsf.survey_type = 'OTHER' AND tsf.resp_val IN ('1' , '2' , '3' , '4') THEN 0
       END
...
FROM
    tmp_sf tsf
    INNER JOIN tmp_maxd tm ON tsf.join_key = tm.join_key
    INNER JOIN tmp_onc_ptnt_exrnc_subsidaries pes ON tsf.survey_id = pes.survey_id
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND 
    tsf.dschrg_dt BETWEEN date_trunc('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-2)) AND (SELECT LAST_DAY(MAX(dschrg_dt)) FROM tmp_sf)
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-3)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-1))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-4)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-2))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-5)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-3))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-6)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-4))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-7)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-5))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-8)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-6))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-9)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-7))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-10)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-8))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-11)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-9))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-12)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-10))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-13)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-11))
GROUP BY 1,2,3
...
WHERE 
    tsf.service='ON' AND 
    tsf.varname IN ('O3') AND
    tsf.dschrg_dt BETWEEN DATE_TRUNC('month',ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-14)) AND LAST_DAY(ADD_MONTHS((SELECT MAX(dschrg_dt) FROM tmp_sf),-12))
GROUP BY 1,2,3);


SELECT 'stg_tmp_op_onc_ptnt_exrnc_pct_msr_fct';
DROP TABLE stg_tmp_op_onc_ptnt_exrnc_pct_msr_fct IF EXISTS;
CREATE TABLE stg_tmp_op_onc_ptnt_exrnc_pct_msr_fct AS 
(
SELECT 
    tspe.service,
    tspe.fcy_nm,
...
FROM
    (
    select service,fcy_nm,rprt_dt,resp_cnt,top_box_resp_cnt from tmp_sub_op_onc_ptnt_exrnc_msr_fct 
    union 
    select service,fcy_nm,rprt_dt,resp_cnt,top_box_resp_cnt from tmp_sub_onc_ptnt_exrnc_msr_fct 
    where fcy_nm in ('Karmanos')
    )tspe
WHERE
    service = 'ON'
GROUP BY 1,2,3);
...
SELECT
    ...
    pepmf.rprt_dt,
    pepmf.resp_cnt,
    pepmf.top_box_resp_cnt,
    pepmf.sub_ptnt_exrnc_pct,
    pepmf.ptnt_exrnc_pct,
    pepd.prgny_pct AS ptnt_exrnc_pct_rnk
FROM
    stg_tmp_op_onc_ptnt_exrnc_pct_msr_fct pepmf
    LEFT OUTER JOIN pce_qe16_pressganey_prd_zoom..prgny_ptnt_exrnc_pct_dim pepd
    ON
        pepmf.service = pepd.svc_cd AND
        pepmf.ptnt_exrnc_pct = pepd.scor AND
        --YEAR(pepmf.rprt_dt) = pepd.reporting_year
        CASE WHEN DATE_PART('MONTH',pepmf.rprt_dt) > 9 THEN YEAR(pepmf.rprt_dt)+1= pepd.reporting_year
        ELSE YEAR(pepmf.rprt_dt) = pepd.reporting_year END
);
...
left join pce_qe16_pressganey_prd_zoom..prgny_ptnt_exrnc_pct_dim ppep on q.service = ppep.svc_cd and --q.ptnt_exrnc_pct = ppep.scor and  year(q.rprt_dt)=ppep.reporting_year --FY23 Change
      CASE WHEN DATE_PART('MONTH',q.rprt_dt) > 9 THEN YEAR(q.rprt_dt)+1= ppep.reporting_year
        ELSE YEAR(q.rprt_dt) = ppep.reporting_year END);
...
FROM  stg_tmp_onc_ptnt_exrnc_pct_msr_fct;
...
drop table tmp_mort_csa IF EXISTS;
create table  tmp_mort_csa as 
(SELECT distinct ef.encntr_num,
ef.fcy_nm,
ef.fcy_num,
ef.stnd_ptnt_type_cd,
...
ef.mrtly_excl_ind,
ef.hspc_pyr_excl,
ef.csa_expc_mrtly_cnt,  --FY23 change
ef.non_covid_case_ind,
ef.expc_mrtly_outc_case_cnt,    -- FY22 addd this column
1 as join_key,
( case when mrtly_outc_case_w_excl=0 then null 
 when mrtly_excl_ind=1  then mrtly_cnt_w_excl
  ...
  (case when mrtly_outc_case_w_excl=0 then null
 when mrtly_excl_ind=1 and hspc_pyr_excl =1 then csa_expc_mrtly_cnt  --FY23 change
 else null end
 ) as mort_expc_cnt
FROM stg_encntr_qs_anl_fct_vw ef
where ef.stnd_ptnt_type_cd = '08' and ef.non_covid_case_ind = 1 and ef.dsc_prsnr_excl_ind =1 
...
select 'tmp_sep_mort_csa';
drop table tmp_sep_mort_csa if exists;     -- new table in FY23. don't re-use tmp_mort
;create table  tmp_sep_mort_csa as 
(
SELECT distinct ef.encntr_num,
ef.fcy_nm,
ef.fcy_num,
...
ef.mrtly_excl_ind,
ef.hspc_pyr_excl,
ef.csa_expc_mrtly_cnt, --FY23 changes
ef.sepsis_icd_ind, --FY23 changes
ef.non_covid_case_ind,
ef.expc_mrtly_outc_case_cnt, 
1 as join_key
FROM stg_encntr_qs_anl_fct_vw ef
...
when trim(fcy_nm) in ('McLaren Thumb Region', 'McLaren Thumb') then 'Thumb'      --FY23 Change
when trim(fcy_nm) in ('McLaren Caro Region Caro Community Hospi', 'McLaren Caro') then 'Caro'    --FY23 Change
else fcy_nm
end as fcy_nm_alias,
rpt_dt,
rpt_prd_end_dt,
...
--CSA Mortality Index
	...
	select 
		'CSA Mortality Index' as measure_name,
		fcy_nm,
		fcy_num,
		date(report_month) as rpt_dt,
		date(report_month) as rpt_prd_end_dt,
	...
	from
		(
			select 
				...
				(sum( case when tm.mrtly_outc_case_w_excl=0 then null
				 when tm.mrtly_excl_ind=1 and tm.hspc_pyr_excl =1 then tm.csa_expc_mrtly_cnt
				 else null end
				)) as mort_expc_agg_cases,
				 
				(sum(CASE WHEN tm.mrtly_excl_ind=1 THEN tm.expc_mrtly_outc_case_cnt else null END ))AS mrtly_outc_cases
				
			from tmp_mort_csa tm
			group by 1,2,3
			order by 1 ASC,2 asc,3 desc
		) mort
...
--FY23 Change
--CSA Mortality Index (R24M)

	select 
		'CSA Mortality Index (R24M)' as measure_name,
		fcy_nm,
		fcy_num,
		date(report_month) as rpt_dt,
		date(report_month) as rpt_prd_end_dt,
		sum(mort_obs_agg_cases) OVER (PARTITION BY fcy_nm ORDER BY date(report_month) ROWS BETWEEN 23 preceding and current row) as obs_cases_r12m,
		sum(mort_expc_agg_cases) OVER (PARTITION BY fcy_nm ORDER BY date(report_month) ROWS BETWEEN 23 preceding and current row) as expc_cases_r12m,
		sum(mrtly_outc_cases) OVER (PARTITION BY fcy_nm ORDER BY date(report_month) ROWS BETWEEN 23 preceding and current row) as outc_cases_r12m,
		obs_cases_r12m/outc_cases_r12m as obs_rt_r12m,
		expc_cases_r12m/outc_cases_r12m as expc_rt_r12m,
		obs_rt_r12m/expc_rt_r12m as oe_rt_r12m,
		mort_obs_agg_cases as obs_agg_cases_mly,
...
--FY23 Change
--CSA Sepsis Mortality Index
	select 
		'CSA Sepsis Mortality Index' as measure_name, 
		fcy_nm,
		fcy_num,
		date(report_month) as rpt_dt,
		date(report_month) as rpt_prd_end_dt,
	...
	(
		select 
			...
			(SUM( case when tm.mrtly_outc_case_w_excl=0 then null
			 when tm.sepsis_icd_ind=1 and tm.hspc_pyr_excl =1 then tm.csa_expc_mrtly_cnt
			 else null end
			)) as mort_expc_agg_cases, 

			count(distinct(case when tm.sepsis_icd_ind=1 and tm.hspc_pyr_excl =1 and tm.expc_mrtly_outc_case_cnt =1 then tm.encntr_num else null end)) as mrtly_outc_cases
		from tmp_sep_mort_csa tm
		group by 1,2,3
		order by 1 ASC,2 asc,3 desc
	) sepmort
...
--FY23 Change
	select 
		'Readmission R24M' as measure_name,
		fcy_nm,
		fcy_num,
		date(report_month) as rpt_dt,
		date(report_month) as rpt_prd_end_dt,
		sum(readm_obs_ct) OVER (PARTITION BY fcy_nm ORDER BY date(report_month) ROWS BETWEEN 23 preceding and current row) as obs_cases_r12m,
		sum(readm_expc_ct) OVER (PARTITION BY fcy_nm ORDER BY date(report_month) ROWS BETWEEN 23 preceding and current row) as expc_cases_r12m,
		sum(readm_out_ct) OVER (PARTITION BY fcy_nm ORDER BY date(report_month) ROWS BETWEEN 23 preceding and current row) as outc_cases_r12m,
		obs_cases_r12m/outc_cases_r12m as obs_rt_r12m,
		expc_cases_r12m/outc_cases_r12m as expc_rt_r12m,
		obs_rt_r12m/expc_rt_r12m as oe_rt_r12m,
		readm_obs_ct as obs_agg_cases_mly,
	...
	(
		select 
		 ...
		 from tmp_readm tr 
		 where fcy_num IN ('823599','MI2194')  --FY23 change (MI2194-Caro , 823599-Thumb)
		 GROUP BY 1,2,3
		 order by 1 ASC,2 asc,3 desc
	) readmin
...
--Critical Access Hospital 	
	select 
		'Critical - Sepsis Bundle Compliance' as measure_name,
		fcy_nm as fcy_nm,
		fcy_num as fcy_num,
		rpt_dt as rpt_dt,
		rpt_dt as rpt_prd_end_dt,
