...
----INFLOW Population
DROP TABLE pce_qe16_slp_prd_dm..inflow_population_hist_data IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..inflow_population_hist_data AS
SELECT DISTINCT
    eaf.fcy_nm,
    eaf.encntr_num,
...
----INFLOW Billing
DROP TABLE pce_qe16_slp_prd_dm..inflow_billing_hist_data IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..inflow_billing_hist_data AS
SELECT
    ...
    COALESCE(eaf.empi,eaf.medical_record_number) AS patient_mrn_identifier,
...
FROM
    ...
    ON
        ...
        eaf.encntr_num = cf.encntr_num
    ------------------------------------------------------------------
    INNER JOIN pce_qe16_slp_prd_dm..inflow_population_hist_data pop
    ON
...
WHERE
    ...
    (DATE(cf.postdate) BETWEEN '2019-10-01' AND edi.bill_txn_end_dt)
DISTRIBUTE ON (fin)
;

-- Inflow Transactions
DROP TABLE pce_qe16_slp_prd_dm..inflow_transactions_hist_data IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..inflow_transactions_hist_data AS
SELECT
    NULL AS charge_id,
...
WHERE
    ...
    (DATE(pftf.postdate) BETWEEN '2019-10-01' AND edi.bill_txn_end_dt)
DISTRIBUTE ON (fin)
;
...
WHERE
    ...
    (DATE(appt_date) BETWEEN '2019-10-01' AND edi.sched_pyrl_end_dt)
