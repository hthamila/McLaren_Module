...
--      ,RAWCHRGCD.charge_code_description as raw_chrg_cd_descr
  ...
  FROM intermediate_stage_temp_eligible_encntr_data Z
  LEFT JOIN cv_patbill CH 
  on Z.company_id = CH.company_id and Z.patient_id = CH.patient_id
  LEFT JOIN cdr_dim hol on to_date(CH.service_date,'mmddyyyy')=hol.cdr_dt
  LEFT JOIN val_set_dim VSET_FCY
  ON VSET_FCY.cd = Z.company_id AND VSET_FCY.cohrt_id = 'FACILITY_CODES'
...
-- 07/21/21: Removed descr column from encntr_anl_Fct since it can be derived from dimension table
	...
	--CODE CHANGE: MLH - 666 : PQSD Discharge Defintion change request
	    WHEN intermediate_stage_temp_dschrg_inpatient_spclcare.dschrg_spclcare_ind =1 AND 
...
DROP TABLE prod_chrg_cost_fct IF EXISTS;
CREATE TABLE prod_chrg_cost_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_chrg_cost_fct;

--select 'processing table: prd_spl_dim ' as table_processing;
DROP TABLE prod_spl_dim IF EXISTS;
CREATE TABLE prod_spl_dim AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_spl_dim;

--select 'processing table: prd_chrg_fct ' as table_processing;
DROP TABLE prod_chrg_fct IF EXISTS;
CREATE TABLE prod_chrg_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_chrg_fct;

--select 'processing table: prd_encntr_cnslt_pract_fct ' as table_processing;
DROP TABLE prod_encntr_cnslt_pract_fct IF EXISTS;
CREATE TABLE prod_encntr_cnslt_pract_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_cnslt_pract_fct;

--select 'processing table:prd_encntr_dgns_fct  ' as table_processing;
DROP TABLE prod_encntr_dgns_fct IF EXISTS;
CREATE TABLE prod_encntr_dgns_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_dgns_fct;

--select 'processing table: prd_encntr_pcd_fct' as table_processing;
DROP TABLE prod_encntr_pcd_fct IF EXISTS;
CREATE TABLE prod_encntr_pcd_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_pcd_fct;

--select 'processing table: prd_encntr_net_rvu_fct ' as table_processing;
DROP TABLE prod_encntr_net_rvu_fct IF EXISTS;
CREATE TABLE prod_encntr_net_rvu_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_net_rvu_fct;

--select 'processing table: prd_encntr_pract_fct ' as table_processing;
DROP TABLE prod_encntr_pract_fct IF EXISTS;
CREATE TABLE prod_encntr_pract_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_pract_fct;

--select 'processing table:prd_svc_ln_anl_fct  ' as table_processing;
DROP TABLE prod_svc_ln_anl_fct if EXISTS;
CREATE TABLE prod_svc_ln_anl_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_svc_ln_anl_fct;

--select 'processing table: prd_encntr_qly_anl_fct ' as table_processing;
DROP TABLE prod_encntr_qly_anl_fct if EXISTS;
CREATE TABLE prod_encntr_qly_anl_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_qly_anl_fct;

--select 'processing table: prd_hist_pymt_ratio ' as table_processing;
DROP TABLE prod_hist_pymt_ratio if EXISTS;
CREATE TABLE prod_hist_pymt_ratio AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_hist_pymt_ratio;

--select 'processing table:prd_hist_pymt_ratio_drg_wghts ' as table_processing;
DROP TABLE prod_hist_pymt_ratio_drg_wghts if EXISTS;
CREATE TABLE prod_hist_pymt_ratio_drg_wghts AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_hist_pymt_ratio_drg_wghts;

--select 'processing table: prd_net_rvu_model ' as table_processing;
DROP TABLE prod_net_rvu_model if EXISTS;
CREATE TABLE prod_net_rvu_model AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_net_rvu_model;

--select 'processing table: prd_cpt_fct ' as table_processing;
DROP TABLE prod_cpt_fct if EXISTS;
CREATE TABLE prod_cpt_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_cpt_fct;

--select 'processing table:prd_encntr_anl_fct  ' as table_processing;
DROP TABLE prod_encntr_anl_fct if EXISTS;
CREATE TABLE prod_encntr_anl_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_anl_fct
DISTRIBUTE ON (fcy_nm,encntr_num);

--select 'processing table:prd_fnc_txn_fct' as table_processing;
DROP TABLE prod_fnc_txn_fct if EXISTS;
CREATE TABLE prod_fnc_txn_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_stage_fnc_txn_fct
DISTRIBUTE ON (fcy_nm,encntr_num);

--select 'processing table:prd_encntr_ed_anl_fct  ' as table_processing;
DROP TABLE prod_encntr_ed_anl_fct if EXISTS;
CREATE TABLE prod_encntr_ed_anl_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_ed_anl_fct;

--select 'processing table:  prd_encntr_oncology_anl_fct' as table_processing;
--DROP TABLE prod_encntr_oncology_anl_fct if EXISTS;
--CREATE TABLE prod_encntr_oncology_anl_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_encntr_oncology_anl_fct;

--select 'processing table: prd_chrg_agg_fct' as table_processing;
DROP TABLE prod_chrg_agg_fct if EXISTS;
CREATE TABLE prod_chrg_agg_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_chrg_agg_fct;

--select 'processing table: prd_ptnt_fnc_txn_agg_fct ' as table_processing;
DROP TABLE prod_ptnt_fnc_txn_agg_fct if EXISTS;
CREATE TABLE prod_ptnt_fnc_txn_agg_fct AS SELECT *,now() as rcrd_isrt_ts FROM  intermediate_ptnt_fnc_txn_agg_fct;

DROP TABLE prod_encntr_prim_cpt_fct if EXISTS;
CREATE TABLE prod_encntr_prim_cpt_fct AS SELECT *,now() as rcrd_isrt_ts FROM intermediate_encntr_prim_cpt_fct;
