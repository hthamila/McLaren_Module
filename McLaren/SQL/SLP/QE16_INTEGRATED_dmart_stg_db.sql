...
--2022-08-09 : PCENB-390 - Allow Pre-registration Encounters through SLP Integrated
select 'processing table:  intermediate_stage_temp_eligible_encntr_data_prereg' as table_processing;
DROP TABLE intermediate_stage_temp_eligible_encntr_data_prereg IF EXISTS;
        CREATE TABLE  intermediate_stage_temp_eligible_encntr_data_prereg AS (
                SELECT DISTINCT ZOOM.company_id
                ,ZOOM.patient_id , ZOOM.inpatient_outpatient_flag ,
                ZOOM.admission_ts
                ,case WHEN ZOOM.inpatient_outpatient_flag ='O' THEN COALESCE(Date(zoom.discharge_ts), Date(zoom.admission_ts)) ELSE Date(zoom.discharge_ts) END AS discharge_ts
        ...
        WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND Date(zoom.admission_ts) BETWEEN  (Select Fiscal_start from intermediate_stage_temp_fiscal_year_tbl) and (select curr_year_end_dt       FROM intermediate_stage_temp_dates_tbl) THEN 'C'
                ...
                WHERE
 coalesce(ZOOM.msdrg_code,'000') NOT IN ('V45','V70') AND
 ZOOM.admissionarrival_date IS NULL
);
...
UNION
SELECT Z.*
FROM intermediate_stage_temp_eligible_encntr_data_prereg Z
;
...
--After CHANGE
		...
		,replace(coalesce(NPIREG.hcare_pvdr_txnmy_descr,NPIREG.hcare_scdy_pvdr_txnmy_descr),'-',' ') as practitioner_spclty_description
                --2022-10-17 - PCENB-609 - Adding Practitioner Sub Speciality Name
                ,COALESCE(NPIREG.hcare_pvdr_txnmy_cl_nm, NPIREG.hcare_scdy_pvdr_txnmy_cl_nm) AS practitioner_sub_spclty_nm
...
, Z.patient_id as encntr_num
      ...
      -- , readmit_hwr42020_risk_adj_num 
      -- , readm_hwr42020_30dy_risk
      -- , readm_hwr42020_30dy_risk_pat_cnt
      -- , readm_hwr42020_30dy_scale_factor
      -- , readmit_hwr42020_unpln_pln_ind
       , ln_los
       , csa_ln_exp_los
       , ln_total_cost
...
--2022-02-04 : To fix the duplicate issue in prd_chrg_fct which was caused due to duplicates from cv_crlinemap_vw
INNER JOIN (SELECT DISTINCT company_id, department_code, costreport_revenue_code_group, cr_line, begin_date, end_date, fiscal_yr from pce_qe16_oper_prd_zoom..cv_crlinemap_vw) CRLINE
...
(
  SELECT Z.company_id as fcy_nm
	...
	,DATE (to_timestamp((CH.postdate || ' ' || nvl(substr(CH.postdate, 1, 2), '00') || ':' || nvl(substr(CH.postdate, 3, 2), '00') || ':00'), 'MMDDYYYY HH24":"MI":"SS')) AS postdate
       	,CH.unitcharge
       	,CH.invoiceid
       	,nvl(CH.performingphysician,'-100') as performingphysician
        ,CH.cpt4full
       	,CH.subaccount
...
--2022-09-09 - PCENB-550 - Colonoscopy Cases for Karmanos
,case when nvl(CH.cpt_code,CHRGCD.cpt_code) IN ('44388', '44389', '44390', '44391', '44392', '44394', '44401', '44402', '44403', '44404', '44405', '44406', '44407', '44408', '45378', '45379', '45380', '45381', '45382', '45384', '45385', '45386', '45388', '45389', '45390', '45391', '45392', '45393', '45398', 'G0105', 'G0121') then 1 else null end as clnscpy_ind
,case when hccs.ccs_hcpcs_cgy_cd='182' then 1 else null end as mamgrphy_ind
--2022-03-22: MLH-949: Imaging fields change starts
, case when (hbt.betos_cd in ('I2C', 'I2D') and ch.revenue_Code in ('0610', '0611', '0612', '0613', '0614', '0615', '0616', '0617', '0618', '0619')) then ch.quantity else null end as imging_mri_qty
, case when (hbt.betos_cd in ('I2A', 'I2B') and ch.revenue_Code in ('0350', '0351', '0352', '0353', '0354', '0355', '0356', '0357', '0358', '0359')) then ch.quantity else null end as imging_ct_qty
, case when (hbt.betos_cd in ('I1A', 'I1B', 'I1F') and ch.revenue_Code in ('0320', '0324', '0329')) then ch.quantity else null  end as imging_stdn_qty
, case when (hbt.betos_cd in ('I1E') and ch.revenue_Code in ('0340', '0341', '0342', '0349')) then ch.quantity else null end as imging_nclrmdcn_qty
, case when (hbt.betos_cd in ('I3A', 'I3B', 'I3E', 'I3F') and ch.revenue_Code = ('0402')) then ch.quantity else null  end as imging_ultrsnd_qty
, case when (hbt.betos_cd in ('I3C', 'I3D') and ch.revenue_Code in ('0480', '0481', '0483', '0360', '0361')) then ch.quantity else null  end as imging_echo_qty
--2022-03-22: MLH-949: Imaging fields change ends
--2022-01-27 : MLH-889: Added HCPCS Information
,CH.hcpcs as hcpcs_cd
,hd.hcpcs_descr as hcpcs_cd_descr
,hd.hcpcs_descr_long as hcpcs_cd_descr_long
,hd.hcpcs_ind as hcpcs_cd_ind
--2022-03-02 : MLH-947,MLH-954 - Adding NPI's for QA Data submissions
,ORDERING.npi as encntr_pcd_ordering_npi
--2022-08-25 : PCENB-454 - Addition of Service Datetime
,CH.service_datetime AS service_dt_tm
--2023-01-16 : PCEB-757 - Addition of I/O Patient Indicator at Charge Level
,CASE WHEN SUBSTRING(CH.subaccount,20) = '415000' THEN 'O' ELSE Z.inpatient_outpatient_flag END AS in_or_out_patient_ind_chrg
--2023-01-30 - PCEB-757 - Addition of MMG Ind in Charge Fact
,CASE WHEN LENGTH(ch.dept) = 9 AND (SUBSTRING(ch.dept,5,5) BETWEEN '50000' AND '58574') THEN 'MMG' ELSE 'Non MMG' END AS mmg_ind_chrg
FROM intermediate_stage_temp_eligible_encntr_data Z
  LEFT JOIN pce_qe16_oper_prd_zoom..cv_patbill CH
  on Z.company_id = CH.company_id and Z.patient_id = CH.patient_id
--2022-12-13    PCENB-818 CHange Logic to use PostDate Instead of Service Date
  LEFT JOIN pce_qe16_slp_prd_dm..cdr_dim hol on to_date(CH.postdate,'mmddyyyy')=hol.cdr_dt
...
LEFT JOIN hcpcs_dim hd on CH.hcpcs = hd.hcpcs_cd
--2022-03-02 : MLH-947,MLH-954 - Adding NPI's for QA Data submissions
LEFT JOIN intermediate_stage_temp_physician_npi_spclty ORDERING
  on ORDERING.company_id = CH.company_id and ORDERING.practitioner_code = CH.ordering_practitioner_code
)
...
--2022-09-09 - PCENB-510 - Addition of AMB DAY SURG 1 HR
--2022-12-05 - PCEB-629 - Addition of raw chargcode for calculated_or_hrs
,CASE WHEN (intermediate_stage_spl_dim.persp_clncl_smy_descr in ('SURGERY TIME','AMBULATORY SURGERY SERVICES') OR(intermediate_stage_chrg_fct_temp.raw_chargcode IN ('SN99042','65000002'))) AND
UPPER(intermediate_stage_spl_dim.persp_clncl_dtl_descr) <> 'OR MINOR FLAT RATE' AND
(UPPER(intermediate_stage_spl_dim.persp_clncl_dtl_descr) IN ('OR MINOR 1 HR','OR MAJOR 1 HR','ROBOTIC OR TIME 1 HOUR','AMB DAY SURG 1 HR') OR (intermediate_stage_spl_dim.cdm_cd in ('3001458200100','3001458200101','4041502857023','4041502857024','4041502857025','4041502857026')))
THEN
...
--2022-03-01 - MLH-953 - Addition of Department Group 2 Field
,dept_dim.department_group2 as dept_grp_2
--2023-01-16 - PCEB-757 - Addition of Combined Facility Name in Charge Fact
,CASE WHEN intermediate_stage_chrg_fct_temp.mmg_ind_chrg = 'MMG' THEN 'MMG' ELSE COALESCE(dept_dim.combined_facility, intermediate_stage_chrg_fct_temp.fcy_nm) END AS cmb_fcy_nm_chrg
FROM intermediate_stage_chrg_fct_temp
...
,cadd.cpt_addnd_b_min_unadj_copymt
--2022-09-09 - PCENB-550 - Colonoscopy Cases for Karmanos
,case when UPPER(CF.cpt_code) IN ('44388', '44389', '44390', '44391', '44392', '44394', '44401', '44402', '44403', '44404', '44405', '44406', '44407', '44408', '45378', '45379', '45380', '45381', '45382', '45384', '45385', '45386', '45388', '45389', '45390', '45391', '45392', '45393', '45398', 'G0105', 'G0121') then 1 else null end as clnscpy_ind
...
--2022-03-02 : MLH-947,MLH-954 - Adding NPI's for QA Data submissions
  LEFT JOIN intermediate_stage_temp_physician_npi_spclty ORDERING
  ON ORDERING.company_id = CF.company_id and ORDERING.practitioner_code = CF.ordering_practitioner_code
 DISTRIBUTE ON (fcy_nm, encntr_num);
...
--PCENB-761 : Radiation Oncology Primary CPT Fix		
        , case when diag_radtn_onclgy_proc_ind='N' then 'Normal Case' else 'Special Case' end as cpt_logic
		, svc_line
		, sub_svc_line
		, svc_nm
...
--PCENB:374 CPT Code Algorithm change -July22
                , CASE WHEN (e.total_charge = 0 AND e.source = 'CHARGE') THEN 0 ELSE 1 END AS chrg_ind
                , ROW_NUMBER() OVER (PARTITION BY e.fcy_nm, e.encntr_num ORDER BY e.cpt_logic desc, chrg_ind DESC, e.cpt_type DESC, rvu DESC,e.total_charge DESC) as rownum
...
SELECT
       ...
       ,CASE
            WHEN sv.inpatient_outpatient_flag = 'I' AND sv.mclaren_major_slp_grouping <> sv.svc_ln AND sv.rec_num = 1 THEN 'Other'
            ELSE sv.sub_svc_ln
        END AS list_sub_svc_line
        ,CASE
            WHEN sv.inpatient_outpatient_flag= 'I' AND sv.mclaren_major_slp_grouping <> sv.svc_ln AND sv.rec_num = 1 THEN 'Other'
            ELSE sv.svc_nm
        END AS list_svc
...
IF EXISTS;
	CREATE TABLE  intermediate_stage_temp_dschrg_inpatient_rehab AS (
		...
		AND Z.inpatient_outpatient_flag = 'I'
		AND (
			lower(admitservice) IN ('rehab','rehabilitation','skilled nursing facility') -- MLH-948 Financial Discharges changes SLP in EDW, Added SNF
	...
	CREATE TABLE  intermediate_stage_temp_dschrg_inpatient_spclcare AS (
		...
		AND (
			...
			OR lower(Z.subfacility) = lower('Bay Special Care')
                        OR lower(Z.patient_type) = lower('Client Inpatien') -- MLH-948 Financial Discharges changes SLP in EDW
                        --2022-04-25: MLH-1010 Financial Discharges handling for Swing Beds
                        OR lower(Z.dischargeservice) = 'sb'
                        OR lower(Z.admitservice) = 'sb'
			) GROUP BY 1
		,2
		);--1440
...
IF EXISTS;
	CREATE TABLE  intermediate_stage_temp_dschrg_inpatient_hospice AS (
		...
		AND Z.inpatient_outpatient_flag = 'I' AND
            (
                VSET_HOSPICE.payor_group3 = 'Hospice' OR
                --2022-05-09: PCENB-89 Hospice Payers at Caro/Thumb (Guarantors)
                (Z.company_id = 'Caro' AND UPPER(Z.dischargeservice) IN ('HSP') AND UPPER(Z.patient_type) = UPPER('HS')) OR
                (Z.company_id = 'Thumb' AND UPPER(Z.dischargeservice) = UPPER('H') AND UPPER(Z.patient_type) = UPPER('IP'))
            )
		AND Z.discharge_total_charges > 0 GROUP BY 1
		,2
		);--4577
...
--2022-12-05 PCEB-629 Addind raw_chargcode and taking the sum of quantity > 0
...
FROM
    ...
    FROM
        intermediate_stage_chrg_fct CF
    INNER JOIN intermediate_stage_spl_dim SP
        ON
        CF.charge_code = SP.cdm_cd
        AND CF.fcy_num = SP.fcy_num
    WHERE
        SP.persp_clncl_smy_descr IN ('SURGERY TIME', 'AMBULATORY SURGERY SERVICES')
        OR CF.raw_chargcode IN ('SN99042', '65000002')
    GROUP BY
        patient_id,
        company_id)A
WHERE
...
SELECT   facility  as fcy_nm
       ...
       --2022-08-08: PCB-136, Updated the ADJUSTMENT transaction type for facility adjustment indicator
       , CASE WHEN UPPER(transtype) in ('A','ADJUSTMENTS','ADJUSTMENT','XFER') THEN 1 ELSE 0 END as fcy_adj_ind
...
---------------------------------------------------------------------------------------
--2022-05-23 : PCENB-96 - Addition of Inpatient Encounter Type Column
--2023-02-06 : PCENB-982 - Updates to Rehab and Special Care
---------------------------------------------------------------------------------------

--select 'processing table:  intermediate_stage_ip_encntr_tp' as table_processing;
DROP TABLE intermediate_stage_ip_encntr_tp IF EXISTS;
CREATE TABLE intermediate_stage_ip_encntr_tp AS
(
SELECT DISTINCT
Z.company_id,
Z.patient_id,
CASE
    WHEN
        (
            LOWER(Z.patient_type) IN ('lip','mip')
        ) THEN NULL
    WHEN
        (
            LOWER(Z.primary_payer_code) IN ('select','selec') OR
            LOWER(Z.patient_type) IN ('bsch','client inpatien','sb','tcu') OR
            LOWER(Z.subfacility) IN ('bay special care') OR
            LOWER(Z.dischargeservice) IN ('sb','long term acute care','skilled nursing facility') OR
            LOWER(Z.admitservice) IN ('sb','long term care')
        ) THEN 'Specialty Care'
    WHEN nvl(PFAGG.excld_trnsfr_encntr_ind,0) = 1 THEN NULL
    WHEN
        (
            LOWER(Z.admitservice) IN ('rehab') OR
            LOWER(Z.dischargeservice) IN ('rehab','rehabilitation') OR
            LOWER(Z.patient_type) IN ('rehab','3')
        ) THEN 'Rehab'
    WHEN
        (
            LOWER(Z.admitservice) IN ('beh','geri','ipsyc','psych') OR
            LOWER(Z.dischargeservice) IN ('beh','geri','ipsyc','behavioral medicine','psych') OR
            LOWER(Z.patient_type) IN ('psych')
        ) THEN 'Psych'
    WHEN
        (
            LOWER(Z.admitservice) IN ('hospice') OR
            LOWER(Z.dischargeservice) IN ('hsp','h','hospice') OR
            LOWER(VSET_HOSPICE.payor_group3) = 'hospice' OR
            LOWER(Z.patient_type) IN ('hs','hospice')
        ) THEN 'Hospice'
    WHEN
        (
            LOWER(Z.patient_type) = 'nb' OR
            LOWER(Z.dischargeservice) IN ('nbn','l1n','bbn','newborn','nurs','n') OR
            LOWER(Z.admitservice) IN ('nb')
        ) THEN 'Newborn'
    WHEN Z.discharge_total_charges > 0 THEN 'Acute'
END AS iptnt_encntr_type,
CASE
    WHEN iptnt_encntr_type IN ('Specialty Care','Rehab','Psych','Hospice','Newborn','Acute') THEN 1
    ELSE NULL
END AS iptnt_dschrg_ind
FROM
    pce_qe16_oper_prd_zoom..cv_patdisch Z
    INNER JOIN intermediate_stage_temp_eligible_encntr_data ENCNTR
    ON
        Z.company_id = ENCNTR.company_id AND
        Z.patient_id = ENCNTR.patient_id
    LEFT OUTER JOIN intermediate_stage_temp_payer_fcy_std_code VSET_HOSPICE
    ON
        VSET_HOSPICE.company_id = Z.company_id AND
        VSET_HOSPICE.payer_code = Z.primary_payer_code
    LEFT OUTER JOIN intermediate_stage_ptnt_fnc_txn_agg_fct PFAGG
    ON
        PFAGG.fcy_nm = Z.company_id AND
        PFAGG.encntr_num = Z.patient_id
WHERE
   -- Z.discharge_date IS NOT NULL AND  (updated to fix Inpatient_Encounter_Type for discharge date NULL)
    Z.inpatient_outpatient_flag = 'I'
);


---------------------------------------------------------------------------------------
--2023-01-04 : PCENB-851 - Addition of Discharge Nursing Department
---------------------------------------------------------------------------------------
--select 'processing table:  intermediate_stage_temp_dschrg_nrsng_dept' as table_processing;
DROP TABLE intermediate_stage_temp_dschrg_nrsng_dept IF EXISTS;
CREATE TABLE intermediate_stage_temp_dschrg_nrsng_dept AS 
(
SELECT
    pb.company_id,
    pb.patient_id,
    MAX(pb.dept || ' - ' || pb.department_description) AS dschrg_nrsng_dept
FROM
    intermediate_stage_chrg_fct pb
INNER JOIN 
( 
SELECT
    company_id,
    patient_id,
    MAX(service_date) AS last_ip_obs_service_date
FROM
    intermediate_stage_chrg_fct 
WHERE
    revenue_code = '0762' OR
    SUBSTRING(revenue_code, 1, 3) IN ('010', '011', '012', '013', '014', '015', '016', '017', '020', '021')
GROUP BY
    company_id,
    patient_id
)sd 
ON 
    sd.company_id = pb.company_id AND
    sd.patient_id = pb.patient_id AND
    sd.last_ip_obs_service_date = pb.service_date
WHERE
    pb.revenue_code = '0762' OR
    SUBSTRING(pb.revenue_code, 1, 3) IN ('010', '011', '012', '013', '014', '015', '016', '017', '020', '021')
GROUP BY
    pb.company_id,
    pb.patient_id
);
...
--2022-08-25 : PCENB-454 - Addition of Source System
	,ZOOM.sourcesystem AS src_sys
	,ZOOM.total_charges_ind AS tot_chrg_ind
	,ZOOM.admitdate_yr_ind AS admdt_yr_ind
	,FCY_REF.bed_cnt
--2022-09-26 : PCENB-96 - Inpatient Encounter Type Changes
    ,CASE
        WHEN ipenctp.patient_id IS NULL THEN NULL
        WHEN ipenctp.iptnt_encntr_type = 'Newborn' THEN 1
        ELSE 0
     END AS dschrg_nbrn_ind
    ,CASE
        WHEN ipenctp.patient_id IS NULL THEN NULL
        WHEN ipenctp.iptnt_encntr_type = 'Rehab' THEN 1
        ELSE 0
     END AS dschrg_rehab_ind
    ,CASE
        WHEN ipenctp.patient_id IS NULL THEN NULL
        WHEN ipenctp.iptnt_encntr_type = 'Psych' THEN 1
        ELSE 0
     END AS dschrg_psych_ind
    ,CASE
        WHEN ipenctp.patient_id IS NULL THEN NULL
        WHEN ipenctp.iptnt_encntr_type = 'Hospice' THEN 1
        ELSE 0
     END AS dschrg_hospice_ind
    ,CASE
        WHEN ipenctp.patient_id IS NULL THEN NULL
        WHEN ipenctp.iptnt_encntr_type = 'Specialty Care' THEN 1
        ELSE 0
     END AS dschrg_spclcare_ind
    ,CASE
        WHEN ipenctp.patient_id IS NULL THEN NULL
        WHEN ipenctp.iptnt_encntr_type = 'Acute' THEN 1
        ELSE 0
     END AS dschrg_acute_ind
    ,ipenctp.iptnt_dschrg_ind AS dschrg_ind
	,CASE
...
-- 07/21/21: Removed descr column from encntr_anl_Fct since it can be derived from dimension table
  ...
  ,nvl(ZOOM.patient_type,'-100') AS ptnt_tp_cd    -- MLH-948 Financial Discharges changes SLP in EDW
-- 07/21/21: Removed descr column from encntr_anl_Fct since it can be derived from dimension table
  ,nvl(PATTYPE.patient_type_description,ZOOM.patient_type,'UNKNOWN') AS ptnt_tp_descr
...
--11/03/2021:MLH-781: Integrated Datamart: Add a new field Recurring Case Indicator
 ...
 rcurrng_case_ind,
--2022-05-19: PCENB-47 - Added the keys for Carenow process
 UPPER(COALESCE(ZOOM.patient_last_name,'') || '|' || SUBSTRING(COALESCE(ZOOM.patient_first_name,''),1,1) || '|' || COALESCE(TO_DATE(ZOOM.birth_date,'MMDDYYYY'),'0001-01-01')) as telmdcn_ptnt_bk,
 HASH8(UPPER(COALESCE(ZOOM.patient_last_name,'') || '|' || SUBSTRING(COALESCE(ZOOM.patient_first_name,''),1,1) || '|' || COALESCE(TO_DATE(ZOOM.birth_date,'MMDDYYYY'),'0001-01-01'))) as telmdcn_ptnt_id,
 UPPER(COALESCE(ZOOM.patient_last_name,'') || '|' || SUBSTRING(COALESCE(ZOOM.patient_first_name,''),1,1) || '|' || COALESCE(TO_DATE(ZOOM.birth_date,'MMDDYYYY'),'0001-01-01') || '|' || COALESCE(DATE(ZOOM.discharge_ts),'0001-01-01')) as telmdcn_encntr_bk,
 HASH8(UPPER(COALESCE(ZOOM.patient_last_name,'') || '|' || SUBSTRING(COALESCE(ZOOM.patient_first_name,''),1,1) || '|' || COALESCE(TO_DATE(ZOOM.birth_date,'MMDDYYYY'),'0001-01-01') || '|' || COALESCE(DATE(ZOOM.discharge_ts),'0001-01-01'))) as telmdcn_encntr_id,
--2022-05-23 : PCENB-96 - Addition of Inpatient Encounter Type Column
 ipenctp.iptnt_encntr_type,
 ipenctp.iptnt_dschrg_ind,
--2022-07-13 : PCEB-96 - Addition of Place of Service
 pos.pos_code,
--2022-08-09 : PCENB-390 - Allow Pre-registration Encounters through SLP Integrated
 CASE WHEN ZOOM.admissionarrival_date IS NULL THEN 1 ELSE 0 END AS prereg_encntr_ind,
--2022-09-01 : PCENB-454 - Addition of Boarder Baby Indicator and Source System
CASE
    WHEN LOWER(ZOOM.dischargeservice) = 'neonatology' AND ZOOM.inpatient_outpatient_flag = 'I' THEN 1 
    ELSE 0 
END AS boarder_baby_ind,
 ZOOM.rcrd_pce_cst_src_nm AS cst_src_nm,
--2022-09-09 : PCENB-510 - Addition of Encounter Date and Reporting Encounter Category
CASE
    WHEN UPPER(ZOOM.patient_type) IN ('PRERECURRING','RCR','RECURRING','RECURRING','SERI','SERIONC','SERIPACE','T','V') THEN TO_DATE(ZOOM.admissionarrival_date,'MMDDYYYY')
    ELSE DATE(ZOOM3YRS.discharge_ts)
END AS encntr_dt,
CASE
    WHEN ZOOM.inpatient_outpatient_flag = 'I' AND (ipenctp.iptnt_encntr_type = 'Newborn' AND ipenctp.iptnt_dschrg_ind = 1) THEN 'Newborn'
    WHEN ZOOM.inpatient_outpatient_flag = 'I' AND (COALESCE(ipenctp.iptnt_encntr_type,'') <> 'Newborn' AND ipenctp.iptnt_dschrg_ind = 1) THEN 'Inpatient'
    WHEN ZOOM.inpatient_outpatient_flag = 'I' AND (dschrg_lipmip_ind = 1) THEN 'Exclude'
    WHEN ZOOM.inpatient_outpatient_flag = 'O' AND (obsrv_stay_ind = 1) THEN 'Observation'
    WHEN ZOOM.inpatient_outpatient_flag = 'O' AND (srgl_case_ind = 1) THEN 'Surgical Day Care'
    WHEN ZOOM.inpatient_outpatient_flag = 'O' AND (ed_case_ind = 1) THEN 'Emergency'
    WHEN ZOOM.inpatient_outpatient_flag = 'O' AND (rcurrng_case_ind = 1) THEN 'Recurring'
    WHEN ZOOM.inpatient_outpatient_flag = 'O' AND (COALESCE(ed_case_ind,0) = 0 AND COALESCE(obsrv_stay_ind,0) = 0 AND COALESCE(rcurrng_case_ind,0) = 0 AND COALESCE(srgl_case_ind,0) = 0 ) THEN 'Other Outpatient'
END AS ar_rpt_encntr_cgy,
--2022-11-02 : PCENB-619 - ISLP Performance Optimization, Moving complex logics from BI to ETL
CASE
    WHEN ed_case_ind = 1 AND ipenctp.iptnt_dschrg_ind = 1 THEN 1
    ELSE 0
END AS ed_admits,
CASE
    WHEN (
            UPPER(ZOOM.patient_type) IN ('BSCH','LIP','BSCHO','MIP','NB' )
            OR UPPER(PRIMPAYER.fcy_payer_code) IN ('SELECT','SELEC')
            OR UPPER(ZOOM.subfacility) = 'BAY SPECIAL CARE'
        )
        OR UPPER(PRIMPAYER.payor_group3) = 'HOSPICE'
        OR (
            UPPER(ZOOM.dischargeservice) IN ('NB','NBN','OIN','SCN','L1N','BBN','NURS','NEWBORN','N','SB')
            OR UPPER(ZOOM.admitservice) IN ( 'NB','NBN', 'OIN','SCN','L1N','BBN','NURS','NEWBORN')
            OR (UPPER(ZOOM.company_id) = 'CARO' AND UPPER(ZOOM.dischargeservice) = 'HSP' AND UPPER(ZOOM.patient_type) = 'HS')
            OR (UPPER(ZOOM.company_id) = 'THUMB' AND UPPER(ZOOM.dischargeservice) = 'H' AND UPPER(ZOOM.patient_type) = 'IP')
        )
        OR ZOOM.patient_type = 'Client Inpatien'
    THEN 0 ELSE 1
END AS fnc_cases,
CASE
    WHEN (UPPER(ZOOM.patient_type) IN ('BSCH', 'BSCHO') OR UPPER(PRIMPAYER.fcy_payer_code) IN ('SELECT', 'SELEC')) OR prereg_encntr_ind = 1 THEN 0
    ELSE 1
END AS tot_cases_excl_sel,
CASE
    WHEN NETREV.est_acct_paid_ind <> ' '
    THEN (CASE WHEN NETREV.est_net_rev_amt > 0 THEN ROUND(NETREV.est_net_rev_amt, 2) ELSE ROUND(0,2) END) - COALESCE(CHRGRCC.agg_rcc_based_direct_cst_amt,0)
    ELSE NULL
END AS contributed_mrgn_amt,
CASE
    WHEN (ZOOM.patient_type  = 'BSCH' OR (PRIMPAYER.fcy_payer_code IN ('SELECT', 'SELEC'))) OR prereg_encntr_ind = 1 THEN NULL
    ELSE ipenctp.iptnt_dschrg_ind
END AS tot_dschrg_excl_sel,
CASE
    WHEN (ZOOM.patient_type = 'BSCH' OR (PRIMPAYER.fcy_payer_code IN ('SELECT', 'SELEC'))) OR prereg_encntr_ind = 1 THEN NULL
    ELSE los
END AS los_all_dschrg_excl_sel,
CASE
    WHEN fnc_cases = 1 THEN COALESCE(ptnt_days_room_n_board_fin_cases,0)
    ELSE 0
END AS rpt_ptnt_days_fin_cases,
dnd.dschrg_nrsng_dept
...
--2022-04-01 - As part of fixing ZOOM mismatches, fcy_nm is used as join criteria for QADV
LEFT JOIN  intermediate_stage_encntr_qly_anl_fct QADV ON ZOOM.patient_id = QADV.encntr_num AND QADV.ptnt_cl_cd = ZOOM.inpatient_outpatient_flag AND ZOOM.company_id = QADV.fcy_nm
...
--2022-05-23 : PCENB-96 - Addition of Inpatient Encounter Type Column
LEFT OUTER JOIN intermediate_stage_ip_encntr_tp ipenctp
ON
    ZOOM.company_id = ipenctp.company_id AND
    ZOOM.patient_id = ipenctp.patient_id
--2022-07-13 : PCEB-96 - Addition of Place of Service
LEFT OUTER JOIN subfacility_2_pos_xwalk pos
ON
    ZOOM.subfacility = pos.subfacility AND
    ZOOM.inpatient_outpatient_flag = (CASE WHEN TRIM(UPPER(pos.inpatientoutpatienttype)) = 'INPATIENT' THEN 'I' WHEN TRIM(UPPER(pos.inpatientoutpatienttype)) = 'OUTPATIENT' THEN 'O' END)
--2023-01-04 : PCENB-851 - Addition of Discharge Nursing Department
LEFT OUTER JOIN intermediate_stage_temp_dschrg_nrsng_dept dnd
ON 
    ZOOM.company_id = dnd.company_id AND
    ZOOM.patient_id = dnd.patient_id
...
--2022-03-22: MLH-949: Imaging fields changes
--Imaging Data Aggregation at encounter level
drop table intermediate_stage_encntr_anl_fct_imgagg_temp IF EXISTS ;
create table intermediate_stage_encntr_anl_fct_imgagg_temp as (
select
	encntr_num
	,sum(imging_mri_qty) as imging_mri_qty
	,sum(imging_ct_qty) as imging_ct_qty
	,sum(imging_stdn_qty) as imging_stdn_qty
	,sum(imging_nclrmdcn_qty) as imging_nclrmdcn_qty
	,sum(imging_ultrsnd_qty) as imging_ultrsnd_qty
	,sum(imging_echo_qty) as imging_echo_qty
from
	intermediate_stage_chrg_fct_temp
group by
	encntr_num) ;
  
--ICU VENT Data Aggregation
DROP TABLE intermediate_stage_encntr_icudata_temp IF EXISTS ;
CREATE TABLE intermediate_stage_encntr_icudata_temp AS
(
    select
        fcy_num
        , encntr_num
        , SUM(case when chest_xray_ind = 1 and icu_ind = 1 then 1 else 0 end ) as imgng_xray_icu
        , SUM(case when chest_xray_ind = 1 and icu_ind = 1 and ventilator_ind = 1 then 1 else 0 end ) as imgng_xray_icu_vent
        --2022-05-05 - MLH-1015: As part of ICU Ventilator Days fix
        --, SUM(case when icu_ind = 1 and ventilator_ind = 1 and ptnt_days_icu > 0 and ptnt_days_ventilator > 0 then 1 else 0 end ) as ptnt_days_icu_vent
        , SUM(case when icu_ind = 1 and ventilator_ind = 1 then 1 else 0 end) as ptnt_days_icu_vent
        from
        (
            select
                cft.fcy_num
                ...
                , cft.patient_id
                , cft.service_date
                , MAX(case when cft.cpt_code in ('71045') then 1 else 0 end) as chest_xray_ind
                , MAX(case when ssd.persp_clncl_smy_descr = 'R&B ICU' then 1 else 0 end) as icu_ind
                , MAX(case when cft.cpt_code in ('94002', '94003') then 1 else 0 end) as ventilator_ind
                , SUM(case when ssd.persp_clncl_smy_descr = 'R&B ICU' then cft.quantity else 0 end) as ptnt_days_icu
                , SUM(case when cft.cpt_code in ('94002', '94003') then cft.quantity else 0 end) as ptnt_days_ventilator
            from
                intermediate_stage_chrg_fct_temp cft
            inner join intermediate_stage_spl_dim ssd
                on
                    cft.charge_code = ssd.cdm_cd and
                    cft.fcy_num = ssd.fcy_num
            group by
                cft.fcy_num,
                cft.encntr_num,
                cft.patient_id,
...
DISTRIBUTE ON (fcy_num, encntr_num);

-------------------------------------- SERVICE LINE Updates & Feb 2022 Imaging Data Inclusion--------------------
...
--2022-03-22: MLH-949: Imaging fields changes
,imgagg.imging_mri_qty
,imgagg.imging_ct_qty
,imgagg.imging_stdn_qty
,imgagg.imging_nclrmdcn_qty
,imgagg.imging_ultrsnd_qty
,imgagg.imging_echo_qty
,icudata.imgng_xray_icu
,icudata.imgng_xray_icu_vent
,icudata.ptnt_days_icu_vent
FROM  intermediate_stage_encntr_anl_fct_temp ef
left join intermediate_stage_encntr_anl_fct_imgagg_temp imgagg on ef.encntr_num = imgagg.encntr_num 
left join intermediate_stage_encntr_icudata_temp icudata on ef.encntr_num = icudata.encntr_num and ef.fcy_num =icudata.fcy_num 
DISTRIBUTE ON (fcy_nm, encntr_num);
...
CREATE TABLE prd_chrg_cost_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_chrg_cost_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);
...
CREATE TABLE prd_chrg_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_chrg_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table: prd_encntr_cnslt_pract_fct ' as table_processing;
DROP TABLE prd_encntr_cnslt_pract_fct IF EXISTS;
CREATE TABLE prd_encntr_cnslt_pract_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_encntr_cnslt_pract_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table:prd_encntr_dgns_fct  ' as table_processing;
DROP TABLE prd_encntr_dgns_fct IF EXISTS;
CREATE TABLE prd_encntr_dgns_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_encntr_dgns_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table: prd_encntr_pcd_fct' as table_processing;
DROP TABLE prd_encntr_pcd_fct IF EXISTS;
CREATE TABLE prd_encntr_pcd_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_encntr_pcd_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table: prd_encntr_net_rvu_fct ' as table_processing;
DROP TABLE prd_encntr_net_rvu_fct IF EXISTS;
CREATE TABLE prd_encntr_net_rvu_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_encntr_net_rvu_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table: prd_encntr_pract_fct ' as table_processing;
DROP TABLE prd_encntr_pract_fct IF EXISTS;
CREATE TABLE prd_encntr_pract_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_encntr_pract_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table:prd_svc_ln_anl_fct  ' as table_processing;
DROP TABLE prd_svc_ln_anl_fct if EXISTS;
CREATE TABLE prd_svc_ln_anl_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_svc_ln_anl_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table: prd_encntr_qly_anl_fct ' as table_processing;
DROP TABLE prd_encntr_qly_anl_fct if EXISTS;
CREATE TABLE prd_encntr_qly_anl_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_encntr_qly_anl_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);
...
CREATE TABLE prd_net_rvu_model AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_net_rvu_model
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table: prd_cpt_fct ' as table_processing;
DROP TABLE prd_cpt_fct if EXISTS;
CREATE TABLE prd_cpt_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_cpt_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table:prd_encntr_anl_fct  ' as table_processing;
DROP TABLE prd_encntr_anl_fct if EXISTS;
CREATE TABLE prd_encntr_anl_fct AS 
	SELECT *,now() as rcrd_isrt_ts, 
		hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey,
		case when isnull(dschrg_dt, now())> '2017-09-30' then  1 else 0 end as fY2018_filter
		--case when  dschrg_dt > cast((year(now())-6) || '-09-30' as date) then  1 else 0 end  fiveyr_data_filter
	FROM  intermediate_encntr_anl_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table:prd_fnc_txn_fct' as table_processing;
DROP TABLE prd_fnc_txn_fct if EXISTS;
CREATE TABLE prd_fnc_txn_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_stage_fnc_txn_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);


select 'processing table:prd_encntr_ed_anl_fct  ' as table_processing;
DROP TABLE prd_encntr_ed_anl_fct if EXISTS;
CREATE TABLE prd_encntr_ed_anl_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_encntr_ed_anl_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);
...
CREATE TABLE prd_chrg_agg_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_chrg_agg_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

select 'processing table: prd_ptnt_fnc_txn_agg_fct ' as table_processing;
DROP TABLE prd_ptnt_fnc_txn_agg_fct if EXISTS;
CREATE TABLE prd_ptnt_fnc_txn_agg_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM  intermediate_ptnt_fnc_txn_agg_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);

DROP TABLE prd_encntr_prim_cpt_fct if EXISTS;
CREATE TABLE prd_encntr_prim_cpt_fct AS SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM intermediate_encntr_prim_cpt_fct
DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);
