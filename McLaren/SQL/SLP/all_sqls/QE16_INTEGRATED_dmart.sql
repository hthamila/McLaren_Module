...
CREATE  TEMP TABLE  intermediate_stage_temp_specl_valid_ind AS 
...
select
        ...
        , surgical_non_surgical cpt_type
        , sum(total_charge) as total_charge
        , 'CHARGE' as source
from intermediate_stage_chrg_fct chrg
        left join xref_hcpcs_svc_ln x on chrg.cpt_code=x.hcpcs
where cpt_code <> '-100'
group by
        fcy_nm
        , encntr_num
...
from intermediate_stage_cpt_fct cpt
        ...
        left join xref_hcpcs_svc_ln x on cpt.cpt_code=x.hcpcs
...
DROP TABLE intermediate_stage_temp_ptnt_prim_n_second_dgns_with_cancer IF EXISTS;
	CREATE TEMP TABLE intermediate_stage_temp_ptnt_prim_n_second_dgns_with_cancer AS (
...
CREATE TEMP TABLE intermediate_stage_temp_table_all_ip_rows  As 
with all_ip_recs as 
(
  select 
...
CREATE TEMP TABLE ip_hist_pymt_ratio_case_a AS 
select  
 --CODE CHANGE : AUG 2019 ; Adding Fiscal Yr in order to calculate the Unpaid Encounters based on Paid Cases
       fiscal_yr,
	   fcy_nm, 
...
CREATE TEMP TABLE ip_hist_pymt_ratio_drg_case_a AS 
select 
--CODE CHANGE : AUG 2019 ; Adding Fiscal Yr in order to calculate the Unpaid Encounters based on Paid Cases
X.fiscal_yr,
X.fcy_nm, 
...
CREATE TEMP TABLE ip_net_rvu_case_a AS 
...
CREATE TEMP TABLE ip_hist_pymt_ratio_case_b AS 
select  
--CODE CHANGE : AUG 2019 ; Adding Fiscal Yr in order to calculate the Unpaid Encounters based on Paid Cases
X.fiscal_yr, 
X.fcy_nm,
...
CREATE TEMP TABLE ip_hist_pymt_ratio_drg_case_b AS 
select 
--CODE CHANGE : AUG 2019 ; Adding Fiscal Yr in order to calculate the Unpaid Encounters based on Paid Cases
X.fiscal_yr,
X.fcy_nm,
...
CREATE TEMP TABLE ip_net_rvu_case_b AS 
-- Paid Cases i.e Account Balance <= 10%
Select 
PAID.* ,
ROUND(PAID.tot_pymt_amt +  abs(PAID.acct_bal_amt), 2 ) as est_net_rev_amt
...
CREATE TEMP TABLE ip_net_rvu_case_c AS 
select ALLCASES.*, 
...
CREATE TEMP TABLE ip_net_rvu_case_d AS 
SELECT
  ALLCASES.fcy_nm, 
...
CREATE TEMP TABLE ip_encntr_net_rvu as 
SELECT * FROM 
...
CREATE TEMP TABLE intermediate_stage_temp_table_all_op_rows As 
with all_op_recs as 
(
  select 
  X.company_id as fcy_nm, 
...
CREATE TEMP TABLE op_hist_pymt_ratio_case_a AS 
select 
 --CODE CHANGE : AUG 2019 ; Adding Fiscal Yr in order to calculate the Unpaid Encounters based on Paid Cases
X.fiscal_yr,
X.fcy_nm, 
...
CREATE TEMP TABLE op_net_rvu_case_a AS 
--Unpaid Cases
select UNPD.*, 
ROUND(UNPD.tot_chrg_amt * PAID.pymt_ratio ,2) as est_net_rev_amt
...
CREATE TEMP TABLE op_hist_pymt_ratio_case_b AS 
select 
--CODE CHANGE : AUG 2019 ; Adding Fiscal Yr in order to calculate the Unpaid Encounters based on Paid Cases
X.fiscal_yr,
X.fcy_nm, 
...
CREATE TEMP TABLE op_net_rvu_case_b AS 
select UNPD.*, 
ROUND(UNPD.tot_chrg_amt * PAID.pymt_ratio,2 )  as est_net_rev_amt
FROM intermediate_stage_temp_table_all_op_rows UNPD
...
CREATE TEMP TABLE op_net_rvu_case_c AS 
select ALLCASES.*, 
ROUND(ALLCASES.tot_chrg_amt * RATIO.pymt_to_chrg_ratio ,2) as est_net_rev_amt
...
CREATE TEMP TABLE op_net_rvu_case_d AS 
SELECT
  ALLCASES.fcy_nm, 
  ALLCASES.in_or_out_patient_ind, 
  ALLCASES.encntr_num, 
...
CREATE TEMP TABLE op_encntr_net_rvu as 
SELECT * FROM 
(select * from op_net_rvu_case_a UNION 
select * from op_net_rvu_case_b UNION 
select * from op_net_rvu_case_c UNION 
...
CREATE TEMP TABLE ip_dept_revenue_charges AS 
...
CREATE TEMP TABLE op_dept_revenue_charges AS 
select T.fcy_nm, T.encntr_num ,T.tot_pymt_amt,T.tot_chrg_amt, T.acct_bal_amt, sum(C.total_charge) as dept_or_revenue_total_charge_amt
,CASE WHEN sum(C.total_charge) > 0 THEN 'Y' ELSE 'N' END as prof_chrg_ind
...
CREATE TEMP TABLE intermediate_stage_encntr_net_rvu_fct_x AS
with combined as 
...
CREATE TEMP TABLE intermediate_stage_temp_encntr_dgns_fct_with_cancer_case AS 
...
CREATE TEMP TABLE intermediate_stage_temp_blood_util_qty AS 
...
CREATE TEMP TABLE intermediate_stage_temp_lab_util_qty AS 
SELECT X.fcy_nm, X.encntr_num,
...
create temp table intermediate_aggr_cpt_fct as
select  fcy_nm, encntr_num,
