...
with temp_lab_or_mammo_ind AS(
(SELECT Distinct
CF.fcy_nm,
CF.encntr_num,
1 as lab_or_mammo_ind
FROM prd_chrg_fct CF WHERE CF.department_group IN ('Lab','Mammography'))
EXCEPT
(SELECT Distinct
CF.fcy_nm,
CF.encntr_num,
1 as lab_or_mammo_ind
FROM prd_chrg_fct CF WHERE CF.department_group NOT IN ('Lab','Mammography'))),
temp_intermediate_stage_chrg_fct_data_for_fcy_lvl_data_only as 
(SELECT
 CF.fcy_nm
,CF.encntr_num
,nvl(EF.empi,EF.medical_record_number) as empi
,EF.dschrg_dt
,EF.adm_ts
...
FROM prd_chrg_fct CF
INNER JOIN prd_encntr_anl_fct EF ON CF.fcy_nm = EF.fcy_nm AND EF.encntr_num = CF.encntr_num
LEFT JOIN intermediate_stage_organ_donor_encntrs_only ORGAN on CF.fcy_nm = ORGAN.fcy_nm AND ORGAN.encntr_num = CF.encntr_num
LEFT JOIN intermediate_stage_onc_region_agg_fct ONCREGN on ONCREGN.encntr_num = CF.encntr_num AND ONCREGN.fcy_nm = CF.fcy_nm
WHERE CF.department_Group is NOT NULL and EF.dschrg_dt is NOT NULL and EF.dschrg_tot_chrg_amt > 0.0
GROUP BY 1,2,3,4,5,6,7 )
select sc.*,
 lm.lab_or_mammo_ind as max_lab_or_mammo_ind
,CASE WHEN (max_organ_donor_ind =1 OR max_lab_or_mammo_ind = 1) AND max_others_ind =0 THEN 0 WHEN (max_organ_donor_ind =0 ) AND max_others_ind =1 THEN 1 ELSE 0 END  as  qualify_visit
...
FROM temp_intermediate_stage_chrg_fct_data_for_fcy_lvl_data_only sc
LEFT JOIN temp_lab_or_mammo_ind lm on sc.fcy_nm = lm.fcy_nm AND sc.encntr_num = lm.encntr_num;
...
CREATE TABLE prd_encntr_anl_pqsd_fct AS 
	SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM intermediate_encntr_anl_pqsd_fct
...
CREATE TABLE prd_encntr_oncology_anl_fct AS 
	SELECT *,now() as rcrd_isrt_ts, hash8(fcy_nm || '-' || encntr_num) as fcy_encntr_hashkey FROM intermediate_encntr_oncology_anl_fct
	DISTRIBUTE ON (fcy_nm,encntr_num,fcy_encntr_hashkey);
