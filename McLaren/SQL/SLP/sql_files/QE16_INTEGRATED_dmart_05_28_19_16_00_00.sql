...
DROP TABLE temp_eligible_encntr_data_inpatient IF EXISTS;
	CREATE TEMP TABLE temp_eligible_encntr_data_inpatient AS (
		...
		,CASE WHEN Date(zoom.discharge_ts) BETWEEN  (Select Fiscal_start from temp_fiscal_year_tbl) and now()- Day(now()) THEN 'C'
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (select DATE(now()- Day(now())) - Interval '1 year' FROM temp_fiscal_year_tbl) THEN 'P'
				      ...
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (select DATE(now()- Day(now())) - Interval '1 year' FROM temp_fiscal_year_tbl)  THEN 'P'
...
( (cast(ZOOM.admission_ts AS DATE) BETWEEN DATE('2015-10-01') AND DATE(now()- Day(now()))) AND (cast(ZOOM.discharge_ts AS DATE) BETWEEN DATE('2015-10-01') AND DATE(now()- Day(now())))) 
...
--CODE Change: Added Discharge ts in the filter based on McLaren's request 
--		(cast(ZOOM.admission_ts AS DATE) >= DATE ('2015-10-01') OR cast(ZOOM.discharge_ts AS DATE) >= DATE ('2015-10-01'))
 (cast(ZOOM.admission_ts AS DATE) BETWEEN DATE('2015-10-01') AND DATE(now()- Day(now()))) 
...
(
  ...
  X.accountbalance as acct_bal_amt,
    ...
    case when X.accountbalance/X.discharge_total_charges * 100 <= 10 THEN
      'Y'
	  ELSE 
...
select ms_drg_cd, CAST(case_mix_idnx_num as NUMERIC(14,2)) as drg_wght
...
Select 
PAID.* ,
ROUND(PAID.tot_pymt_amt +  PAID.acct_bal_amt, 2) as est_net_rev_amt
...
on UNPD.ms_drg_cd  = PAID.ms_drg_cd and UNPD.src_prim_payor_grp1 = PAID.src_prim_payor_grp1 and UNPD.fcy_nm = PAID.fcy_nm 
...
--	,nvl(PRIMPROC.prim_proc_icd_pcd_descr,'UNKNOWN') as prim_pcd_descr
	...
	,ROUND(NETREV.est_net_rev_amt, 2) as est_net_rev_amt
	...
	, CANCER.cancer_case_code_descr as cancer_case_code_descr
...
LEFT JOIN pce_qe16_slp_prd_dm..dgns_dim ADMDGNS ON ADMDGNS.dgns_alt_cd = replace(ZOOM.admitdiagnosiscode,'.','')
