...
DROP TABLE intermediate_temp_fiscal_year_tbl IF EXISTS; 
CREATE Table intermediate_temp_fiscal_year_tbl AS 
...
DROP TABLE intermediate_temp_dates_tbl IF EXISTS; 

CREATE TEMP TABLE intermediate_temp_dates_tbl 
AS 
select 
...
DROP TABLE intermediate_temp_eligible_encntr_data_inpatient IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_eligible_encntr_data_inpatient AS (
		...
		,CASE WHEN Date(zoom.discharge_ts) BETWEEN  (Select Fiscal_start from intermediate_temp_fiscal_year_tbl) and --now()- Day(now()) THEN 'C'
		 (select curr_year_end_dt from intermediate_temp_dates_tbl) THEN 'C' 
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '1 year' FROM intermediate_temp_dates_tbl) THEN 'P'
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '2 year' from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '2 year' FROM intermediate_temp_dates_tbl) THEN 'P-1'
					  WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND Date(zoom.admission_ts) BETWEEN  (Select Fiscal_start from intermediate_temp_fiscal_year_tbl) and 
                       (select curr_year_end_dt	FROM intermediate_temp_dates_tbl)				  THEN 'C'
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '1 year' FROM intermediate_temp_dates_tbl)  THEN 'P'
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '2 year' from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '2 year' FROM intermediate_temp_dates_tbl)  THEN 'P-1'                       
...
SELECT count(*)
FROM intermediate_temp_eligible_encntr_data_inpatient;

DROP TABLE intermediate_temp_eligible_encntr_data_outpatient IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_eligible_encntr_data_outpatient AS (
		SELECT DISTINCT ZOOM.company_id
		,ZOOM.patient_id , ZOOM.inpatient_outpatient_flag ,
		ZOOM.admission_ts
		,case WHEN ZOOM.inpatient_outpatient_flag ='O' THEN COALESCE(Date(zoom.discharge_ts), Date(zoom.admission_ts)) ELSE Date(zoom.discharge_ts) END AS discharge_ts
        ...
        ,CAST(NULL as NUMERIC(14,4)) as ms_drg_arthm_mean_los_num
                ,CASE WHEN  MONTH(date_trunc('quarter',date(discharge_ts))) >= 10  THEN 'FY' || (YEAR(discharge_ts) + 1) 
					   ...
					   ELSE
					   NULL END as fiscal_yr
		,CASE WHEN Date(zoom.discharge_ts) BETWEEN  (Select Fiscal_start from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt from intermediate_temp_dates_tbl) THEN 'C' 
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '1 year' FROM intermediate_temp_dates_tbl) THEN 'P'
	                  WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '2 year' from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '2 year' FROM intermediate_temp_dates_tbl) THEN 'P-1'
	WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND Date(zoom.admission_ts) BETWEEN  (Select Fiscal_start from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt	FROM intermediate_temp_dates_tbl) THEN 'C'
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '1 year' FROM intermediate_temp_dates_tbl)  THEN 'P'
                      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '2 year' from intermediate_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '2 year' FROM intermediate_temp_dates_tbl)  THEN 'P-1'                                     
...
SELECT count(*)
FROM intermediate_temp_eligible_encntr_data_outpatient;

--CODE CHANGE : AUG 2019 (a) Ms_Drg_Dim Historical CMI Weights
DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_ms_drg_dim_hist  IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_ms_drg_dim_hist AS 
select ms_drg_cd, 
...
--CODE CHANGE : AUG 2019 (a) Ms_Drg_Dim Historical CMI Weights   intermediate_temp_eligible_encntr_data
DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_eligible_encntrs IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_eligible_encntrs 
AS 
SELECT X.*
FROM intermediate_temp_eligible_encntr_data_inpatient X
UNION 
SELECT Y.*
FROM intermediate_temp_eligible_encntr_data_outpatient Y; 
		
SELECT count(*)
FROM intermediate_temp_eligible_encntrs;

DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_encntr_with_ms_drg_wghts IF EXISTS; 

CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_encntr_with_ms_drg_wghts AS 
SELECT 
...
FROM pce_qe16_slp_prd_dm..intermediate_temp_eligible_encntrs X 
INNER JOIN pce_qe16_slp_prd_dm..intermediate_temp_ms_drg_dim_hist DRGWGHT
ON X.msdrg_code= DRGWGHT.ms_drg_cd  AND date(X.discharge_ts)  BETWEEN DRGWGHT.vld_fm_dt AND DRGWGHT.vld_to_dt;


DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_encntr_without_ms_drg_wghts IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_encntr_without_ms_drg_wghts AS 
with recs_with_weights AS 
(select distinct patient_id , company_id FROM pce_qe16_slp_prd_dm..intermediate_temp_encntr_with_ms_drg_wghts)
select * FROM pce_qe16_slp_prd_dm..intermediate_temp_eligible_encntrs X  
WHERE (patient_id || company_id) NOT IN (select (patient_id || company_id) from recs_with_weights );

DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_eligible_encntr_data IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_eligible_encntr_data 
AS 
SELECT X.*
FROM intermediate_temp_encntr_with_ms_drg_wghts X
UNION 
SELECT Y.*
FROM intermediate_temp_encntr_without_ms_drg_wghts Y; 
		
SELECT count(*)
FROM intermediate_temp_eligible_encntr_data;


----------------------------------------


DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_physician_npi_spclty

IF EXISTS;
	CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_physician_npi_spclty AS (
...
Z.company_id as src_company_id, Z.patient_id as src_patient_id , QADV.* from intermediate_temp_eligible_encntr_data Z
...
--	   ,RAWCHRGCD.charge_code_description as raw_chrg_cd_descr
  FROM intermediate_temp_eligible_encntr_data Z
...
Order by  CH.service_start_date) as rec_num
  ...
  on Z.company_id = CH.company_id and Z.patient_id = CH.patient_id
  LEFT JOIN intermediate_temp_physician_npi_spclty SPCL
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_specl_valid_ind IF EXISTS ; 
CREATE  TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_specl_valid_ind AS 
select distinct P.src_company_id, P.src_patient_id, 1 as specl_valid_ind
FROM intermediate_encntr_pract_fct P 
INNER JOIN intermediate_temp_eligible_encntr_data A 
on A.company_id = P.src_company_id and A.patient_id = P.src_patient_id and A.inpatient_outpatient_flag = 'I'
INNER JOIN intermediate_temp_physician_npi_spclty S
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_dgns_ccs_dim_cancer_only IF EXISTS;
CREATE TEMP TABLE intermediate_temp_dgns_ccs_dim_cancer_only AS 
...
Order by  DF.diagnosisseq) as rec_num 
  ...
  LEFT JOIN pce_qe16_slp_prd_dm..val_set_dim VSET_FCY
  ON VSET_FCY.cd = Z.company_id AND VSET_FCY.cohrt_id = 'FACILITY_CODES'
  LEFT JOIN pce_qe16_slp_prd_dm..intermediate_temp_dgns_ccs_dim_cancer_only CANCER
...
--       , DF.diagnosistype
  ...
  LEFT JOIN pce_qe16_slp_prd_dm..val_set_dim VSET_FCY
  ON VSET_FCY.cd = Z.company_id AND VSET_FCY.cohrt_id = 'FACILITY_CODES'
  LEFT JOIN intermediate_temp_physician_npi_spclty SURGEON
  on SURGEON.company_id = DF.company_id and SURGEON.practitioner_code = DF.surgeon_code
    LEFT JOIN intermediate_temp_physician_npi_spclty ORDERING
...
DROP TABLE intermediate_temp_svc_based_on_patbill IF exists; 
CREATE TEMP TABLE intermediate_temp_svc_based_on_patbill as 
select  
...
from intermediate_temp_eligible_encntr_data EN
...
DROP TABLE intermediate_temp_svc_based_on_pcdfct IF exists; 
CREATE TEMP TABLE intermediate_temp_svc_based_on_pcdfct as 
select  
hash8(EN.company_id) as src_company_id_hash, hash8(EN.patient_id) as src_patient_id_hash,
...
--?????
--Based on patcpt 
DROP TABLE intermediate_temp_svc_based_on_patcpt IF exists; 
CREATE TEMP TABLE intermediate_temp_svc_based_on_patcpt as 
select  
hash8(EN.company_id) as src_company_id_hash, hash8(EN.patient_id) as src_patient_id_hash,
EN.company_id, EN.patient_id ,EN.inpatient_outpatient_flag, 'cpt' as based_on,
--nvl(RNK.cd,'-100'),
...
--MS-DRG (Inpatient)
DROP TABLE intermediate_temp_svc_based_on_patdisch IF exists; 
CREATE TEMP TABLE intermediate_temp_svc_based_on_patdisch as 
select  
hash8(EN.company_id) as src_company_id_hash, hash8(EN.patient_id) as src_patient_id_hash,
...
RNK.svc_cgy,RNK.svc_ln,RNK.sub_svc_ln , RNK.services as svc_nm, RNK.cd, RNK.cd_type,RNK.descr as cd_descr,
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
from intermediate_temp_eligible_encntr_data X
...
with  all_joined as (
select * from intermediate_temp_svc_based_on_patbill union   -- //HCPCS / CPT 
select * from intermediate_temp_svc_based_on_patdisch union  --Ms-Drg-code
select * from intermediate_temp_svc_based_on_pcdfct  union   --ICD 9/10 PCS
select * from intermediate_temp_svc_based_on_patcpt   --CPT 
 ),
...
DROP TABLE intermediate_temp_encntr_svc_hier IF EXISTS;
CREATE TEMP TABLE intermediate_temp_encntr_svc_hier AS 
...
--
DROP TABLE intermediate_temp_fips_adr_dim IF EXISTS; 
CREATE TEMP TABLE intermediate_temp_fips_adr_dim as 
(
   select 
...
DROP TABLE intermediate_temp_ptnt_type_fcy_std_cd

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ptnt_type_fcy_std_cd AS (
...
DROP TABLE intermediate_temp_ptnt_adm_dgns

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ptnt_adm_dgns AS (
		SELECT Z.company_id
		,Z.patient_id
		...
		INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON ENCNTR.company_id = X.company_id
...
DROP TABLE intermediate_temp_ptnt_prim_dgns

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ptnt_prim_dgns AS (
		SELECT Z.company_id
		,Z.patient_id
...
DROP TABLE intermediate_temp_ptnt_second_dgns

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ptnt_second_dgns AS (
		SELECT Z.company_id
		,Z.patient_id
...
SELECT count(*)
FROM intermediate_temp_ptnt_second_dgns;--4,432,358

DROP TABLE intermediate_temp_ptnt_trty_dgns

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ptnt_trty_dgns AS (
		SELECT Z.company_id
		,Z.patient_id
...
DROP TABLE intermediate_temp_ptnt_prim_proc

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ptnt_prim_proc AS (
		SELECT Z.company_id
		,Z.patient_id
...
SELECT count(*)
FROM intermediate_temp_ptnt_prim_proc;--332,451

DROP TABLE intermediate_temp_ptnt_scdy_proc

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ptnt_scdy_proc AS (
		SELECT Z.company_id
		,Z.patient_id
...
SELECT count(*)
FROM intermediate_temp_ptnt_scdy_proc;--145,743

DROP TABLE intermediate_temp_ptnt_trty_proc

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ptnt_trty_proc AS (
		SELECT Z.company_id
		,Z.patient_id
...
DROP TABLE intermediate_temp_obsrv

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_obsrv AS (
...
DROP TABLE intermediate_temp_icu

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_icu AS (
...
--		 INNER JOIN intermediate_temp_dschrg_inpatient ON intermediate_temp_dschrg_inpatient.patient_id = ZOOM.patient_id 
...
--Code Change : Modified the existing logic (Rev Code) based on SPL Dimension
DROP TABLE intermediate_temp_ccu

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ccu AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
DROP TABLE intermediate_temp_nrs

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_nrs AS (
		SELECT patient_id
		,company_id
...
DROP TABLE intermediate_temp_rtne

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_rtne AS (
		SELECT patient_id
		,company_id
...
DROP TABLE intermediate_temp_ed_case

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_ed_case AS (
...
DROP TABLE intermediate_temp_dschrg_inpatient_ltcsnf

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_ltcsnf AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_ltcsnf_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
...
DROP TABLE intermediate_temp_dschrg_inpatient_nbrn

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_nbrn AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_nbrn_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I'
		AND (
...
DROP TABLE intermediate_temp_dschrg_inpatient_rehab

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_rehab AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_rehab_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I'
		AND (
...
DROP TABLE intermediate_temp_dschrg_inpatient_psych

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_psych AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_psych_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I'
		AND (
...
DROP TABLE intermediate_temp_dschrg_inpatient_spclcare

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_spclcare AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
...
--DROP TABLE intermediate_temp_dschrg_inpatient_hospice
--
--IF EXISTS;
--	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_hospice AS (
--		SELECT DISTINCT Z.patient_id
--		,Z.company_id
--		,1 AS dschrg_hospice_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
...
DROP TABLE intermediate_temp_payer_fcy_std_code

IF EXISTS;
--	CREATE TEMP TABLE intermediate_temp_payer_fcy_std_code AS (
...
IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_hospice AS (
		...
		INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id
		INNER JOIN intermediate_temp_payer_fcy_std_code VSET_HOSPICE
...
DROP TABLE intermediate_temp_dschrg_inpatient_lipmip

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_lipmip AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
...
DROP TABLE intermediate_temp_dschrg_inpatient_acute

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient_acute AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,CASE 
			WHEN (
				...
				THEN NULL
			ELSE 1
			END AS dschrg_acute_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id LEFT JOIN intermediate_temp_dschrg_inpatient_nbrn NB ON NB.patient_id = Z.patient_id
		AND NB.company_id = Z.company_id LEFT JOIN intermediate_temp_dschrg_inpatient_lipmip LIPMIP ON LIPMIP.patient_id = Z.patient_id
		AND LIPMIP.company_id = Z.company_id LEFT JOIN intermediate_temp_dschrg_inpatient_rehab REHAB ON REHAB.patient_id = Z.patient_id
		AND REHAB.company_id = Z.company_id LEFT JOIN intermediate_temp_dschrg_inpatient_psych PSYCH ON PSYCH.patient_id = Z.patient_id
		AND PSYCH.company_id = Z.company_id LEFT JOIN intermediate_temp_dschrg_inpatient_ltcsnf LTCSNF ON LTCSNF.patient_id = Z.patient_id
		AND LTCSNF.company_id = Z.company_id LEFT JOIN intermediate_temp_dschrg_inpatient_hospice HOSPICE ON HOSPICE.patient_id = Z.patient_id
		AND HOSPICE.company_id = Z.company_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I'
		AND Z.discharge_total_charges > 0
		);--323191 

DROP TABLE intermediate_temp_dschrg_inpatient

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_dschrg_inpatient AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I' GROUP BY 1
		,2
		);--323191

DROP TABLE intermediate_temp_derived_ptnt_days

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_derived_ptnt_days AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
		,CASE WHEN (sum(ZOOM.quantity) =0) THEN NULL ELSE sum(ZOOM.quantity) END  AS ptnt_days 
		FROM pce_qe16_slp_prd_dm..intermediate_chrg_fct ZOOM INNER JOIN intermediate_temp_dschrg_inpatient ON intermediate_temp_dschrg_inpatient.patient_id = ZOOM.patient_id WHERE (
			(
...
DROP TABLE intermediate_temp_endoscopy_case

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_endoscopy_case AS (
		SELECT DISTINCT patient_id
...
DROP TABLE intermediate_temp_srgl_case

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_srgl_case AS (
		SELECT DISTINCT patient_id
...
--code change : Added logic to calculate  Lithotripsy  Cases based on SPL Dimension
   DROP TABLE intermediate_temp_lithotripsy_case IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_lithotripsy_case AS (
		SELECT DISTINCT patient_id
		,company_id FROM pce_qe16_slp_prd_dm..intermediate_chrg_fct CF 
		INNER JOIN pce_qe16_slp_prd_dm..intermediate_spl_dim SP
		on CF.charge_code = SP.cdm_cd and CF.fcy_num = SP.fcy_num
 ...
 --code change : Added logic to calculate  CathLab Cases based on intermediate_svc_ln_anl_fct
    DROP TABLE intermediate_temp_cathlab_case IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_cathlab_case AS
	(
...
DROP TABLE intermediate_temp_derived_ptnt_days_stepdown

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_derived_ptnt_days_stepdown AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
DROP TABLE intermediate_temp_derived_ptnt_days_nbrn

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_derived_ptnt_days_nbrn AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
DROP TABLE intermediate_temp_derived_ptnt_days_psych

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_derived_ptnt_days_psych AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
--CODE change: Modified the existing logic (REv Code) based on SPL Dimension
DROP TABLE intermediate_temp_derived_ptnt_days_rehab

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_derived_ptnt_days_rehab AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
DROP TABLE intermediate_temp_derived_ptnt_days_acute IF EXISTS; 


--Code Change: Modified the logic to calculate ptnt_days_acute
DROP TABLE intermediate_temp_derived_ptnt_days_acute

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_derived_ptnt_days_acute AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
--		 FROM pce_qe16_slp_prd_dm..intermediate_chrg_fct ZOOM 
--		INNER JOIN intermediate_temp_dschrg_inpatient ON intermediate_temp_dschrg_inpatient.patient_id = ZOOM.patient_id WHERE (
...
FROM intermediate_temp_eligible_encntr_data T1
...
DROP TABLE intermediate_temp_surgeon_pract IF exists;
CREATE TEMP TABLE intermediate_temp_surgeon_pract AS 
(
  ...
  LEFT JOIN pce_qe16_slp_prd_dm..intermediate_temp_physician_npi_spclty PHY
...
DROP TABLE intermediate_temp_physician_fcy_std_spclty

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_physician_fcy_std_spclty AS (
		SELECT DISTINCT PT.company_id
		,PT.practitioner_code
		,PT.practitioner_name
...
DROP TABLE intermediate_temp_discharge_fcy_std_status_code

IF EXISTS;
	CREATE TEMP TABLE intermediate_temp_discharge_fcy_std_status_code AS (
		SELECT DISTINCT ZOOM.discharge_status
		,DISSTATUS.dschrg_sts_cd
		,DISSTATUS.dschrg_sts_descr FROM pce_qe16_oper_prd_zoom..cv_patdisch ZOOM INNER JOIN intermediate_temp_eligible_encntr_data ENCNTR ON ZOOM.company_id = ENCNTR.company_id
...
----Qualifiers 
--DROP TABLE intermediate_temp_eligible_encntr_data IF EXISTS;
--	CREATE TEMP TABLE intermediate_temp_eligible_encntr_data AS (
--		SELECT DISTINCT ZOOM.company_id
--		,ZOOM.patient_id , ZOOM.inpatient_outpatient_flag ,
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_table_all_ip_rows IF Exists; 
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_table_all_ip_rows  As 
with all_ip_recs as 
(
  select 
...
from pce_qe16_oper_prd_zoom..cv_patdisch X 
 ...
 INNER JOIN intermediate_temp_eligible_encntr_data Z
...
FROM intermediate_temp_table_all_ip_rows Z
...
and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now())) 
--Code Change : 08/08 As per McLaren Requet adding patient_Type exclusion
and  upper(Z.ptnt_tp_Cd) NOT in ('LIP','MIP')
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30')
Group by 1,2,3,4) X;
...
--     ROUND(sum(tot_chrg_amt),2) as tot_chrg_amt
FROM intermediate_temp_table_all_ip_rows Z
INNER JOIN intermediate_temp_ms_drg_dim_hist MSDRG
...
FROM intermediate_temp_table_all_ip_rows PAID
WHERE PAID.est_acct_paid_ind ='Y' and PAID.src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid')  
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now())) 
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30') --85,855 Records
...
from intermediate_temp_table_all_ip_rows UNPD
LEFT JOIN intermediate_temp_ms_drg_dim_hist DRGWGHT
...
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30') --4578 Unpaid Cases with a Drg
UNION
...
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now())) 
;
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30');  --26 Records Unpaid Cases without Drg
...
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now())) 
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30') --7,559 Records
-- Unpaid Cases with a Drg (i.e Payments would be calculated based on Historical DRG Weights Ratio)
UNION 
select UNPD.*, 
ROUND(DRGWGHT.drg_wght * PAID.drg_weighted_pmnt_per_case, 2)  as est_net_rev_amt
...
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now()))
;
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30');  --4 Records Unpaid Cases without Drg
...
FROM intermediate_temp_table_all_ip_rows ALLCASES
...
--WHERE cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now())) 
;
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30')
--AND fcy_nm != 'Lansing';
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_table_all_op_rows IF Exists; 
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_temp_table_all_op_rows As 
with all_op_recs as 
(
  select 
  X.company_id as fcy_nm, 
...
--ROUND(sum(tot_pymt_amt ) ,2) as  tot_pymt_amt,
--ROUND(sum(tot_chrg_amt),2) as tot_chrg_amt
FROM intermediate_temp_table_all_op_rows Z
WHERE est_acct_paid_ind ='Y' and src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid')  
--Code Change : 08/08 As per McLaren Requet adding patient_Type exclusion
and upper(Z.ptnt_tp_Cd) NOT in ('LIP','MIP')
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now())) 
-- and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now())) 
...
select UNPD.*, 
ROUND(UNPD.tot_chrg_amt * PAID.pymt_ratio ,2) as est_net_rev_amt
FROM intermediate_temp_table_all_op_rows UNPD
...
ROUND(PAID.tot_pymt_amt +  abs(PAID.acct_bal_amt) ,2 ) as est_net_rev_amt
FROM intermediate_temp_table_all_op_rows PAID
WHERE PAID.est_acct_paid_ind ='Y' and PAID.src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid')  
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_temp_fiscal_year_tbl) and (now()- Day(Now())) 
;
...
select ALLCASES.*, 
ROUND(ALLCASES.tot_chrg_amt * RATIO.pymt_to_chrg_ratio ,2) as est_net_rev_amt
FROM intermediate_temp_table_all_op_rows ALLCASES
LEFT JOIN pce_qe16_slp_prd_dm..manual_pymt_chrg_ratio RATIO
on RATIO.payor_group_1  = ALLCASES.src_prim_payor_grp1 AND RATIO.company_id = ALLCASES.fcy_nm
WHERE ALLCASES.src_prim_payor_grp1 in ('Domestic')  
...
FROM intermediate_temp_table_all_ip_rows T
...
select T.fcy_nm, T.encntr_num ,T.tot_pymt_amt,T.tot_chrg_amt, T.acct_bal_amt, sum(C.total_charge) as dept_or_revenue_total_charge_amt
,CASE WHEN sum(C.total_charge) > 0 THEN 'Y' ELSE 'N' END as prof_chrg_ind
FROM intermediate_temp_table_all_op_rows T
INNER JOIN pce_qe16_slp_prd_dm..intermediate_chrg_fct C
on C.company_id = T.fcy_nm and T.encntr_num = C.patient_id
WHERE (
--Department Exclusion
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_temp_encntr_dgns_fct_with_cancer_case IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_temp_encntr_dgns_fct_with_cancer_case AS 
...
SELECT Distinct
	...
	,ZOOM3YRS.ms_drg_wght AS case_mix_idnx_num_encntr_lvl
  ...
  --,ZOOM.subfacility AS sub_fcy
	...
	,FCY_REF.bed_cnt
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient_nbrn.patient_id IS NULL
			THEN NULL
		ELSE intermediate_temp_dschrg_inpatient_nbrn.dschrg_nbrn_ind
		END AS dschrg_nbrn_ind
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient_rehab.patient_id IS NULL
			THEN NULL
		ELSE intermediate_temp_dschrg_inpatient_rehab.dschrg_rehab_ind
		END AS dschrg_rehab_ind
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient_psych.patient_id IS NULL
			THEN NULL
		ELSE intermediate_temp_dschrg_inpatient_psych.dschrg_psych_ind
		END AS dschrg_psych_ind
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient_ltcsnf.patient_id IS NULL
			THEN NULL
		ELSE intermediate_temp_dschrg_inpatient_ltcsnf.dschrg_ltcsnf_ind
		END AS dschrg_ltcsnf_ind
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient_hospice.patient_id IS NULL
			THEN NULL
		ELSE intermediate_temp_dschrg_inpatient_hospice.dschrg_hospice_ind
		END AS dschrg_hospice_ind
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient_spclcare.patient_id IS NULL
			THEN NULL
		ELSE intermediate_temp_dschrg_inpatient_spclcare.dschrg_spclcare_ind
		END AS dschrg_spclcare_ind
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient_lipmip.patient_id IS NULL
			THEN NULL
		ELSE intermediate_temp_dschrg_inpatient_lipmip.dschrg_lipmip_ind
		END AS dschrg_lipmip_ind
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient_acute.patient_id IS NULL
			THEN NULL
		ELSE intermediate_temp_dschrg_inpatient_acute.dschrg_acute_ind
		END AS dschrg_acute_ind
	,CASE 
		WHEN intermediate_temp_dschrg_inpatient.patient_id IS NOT NULL
		...
		END AS dschrg_ind
	,CASE 
		WHEN intermediate_temp_obsrv.qty > 0
			THEN intermediate_temp_obsrv.qty
		ELSE intermediate_temp_obsrv.qty
		END AS obsrv_hours
	,CASE 
		WHEN obsrv_hours > 0
			THEN obsrv_hours / 24
	...
	, ( nvl(intermediate_temp_derived_ptnt_days_acute.ptnt_days_acute ,0)+ 
	   nvl(intermediate_temp_ccu.ccu_days, 0) + 
	   nvl(intermediate_temp_icu.icu_days, 0) + 
	   nvl(intermediate_temp_derived_ptnt_days_stepdown.ptnt_days_stepdown, 0) +
	   nvl(intermediate_temp_derived_ptnt_days_nbrn.ptnt_days_nbrn, 0) + 
	   nvl(intermediate_temp_derived_ptnt_days_rehab.ptnt_days_rehab, 0) +
	   nvl(intermediate_temp_derived_ptnt_days_psych.ptnt_days_psych, 0)) AS ptnt_days
	,CASE 
		WHEN intermediate_temp_derived_ptnt_days_psych.patient_id IS NOT NULL
			THEN intermediate_temp_derived_ptnt_days_psych.ptnt_days_psych
		ELSE NULL
		END AS ptnt_days_pysch
	,CASE 
		WHEN intermediate_temp_derived_ptnt_days_rehab.patient_id IS NOT NULL
			THEN intermediate_temp_derived_ptnt_days_rehab.ptnt_days_rehab
		ELSE NULL
		END AS ptnt_days_rehab
	,CASE 
		WHEN intermediate_temp_derived_ptnt_days_nbrn.patient_id IS NOT NULL
			THEN intermediate_temp_derived_ptnt_days_nbrn.ptnt_days_nbrn
		ELSE NULL
		END AS ptnt_days_nbrn
	,CASE 
		WHEN intermediate_temp_derived_ptnt_days_stepdown.patient_id IS NOT NULL
			THEN intermediate_temp_derived_ptnt_days_stepdown.ptnt_days_stepdown
		ELSE NULL
		END AS ptnt_days_stepdown
	,CASE 
		WHEN intermediate_temp_derived_ptnt_days_acute.patient_id IS NOT NULL
			THEN intermediate_temp_derived_ptnt_days_acute.ptnt_days_acute
		ELSE NULL
		END AS ptnt_days_acute
	,CASE 
		WHEN intermediate_temp_icu.patient_id IS NOT NULL
			THEN intermediate_temp_icu.icu_days
		ELSE NULL
		END AS icu_days
	,CASE 
		WHEN intermediate_temp_ccu.patient_id IS NOT NULL
			THEN intermediate_temp_ccu.ccu_days
		ELSE NULL
		END AS ccu_days
	,CASE 
		WHEN intermediate_temp_nrs.patient_id IS NOT NULL
			THEN intermediate_temp_nrs.nrs_days
		ELSE NULL
		END AS nrs_days
	,CASE 
		WHEN intermediate_temp_rtne.patient_id IS NOT NULL
			THEN intermediate_temp_rtne.rtne_days
		ELSE NULL
		END AS rtne_days
	,CASE 
		WHEN intermediate_temp_ed_case.patient_id IS NOT NULL
			THEN 1
		ELSE NULL
		END AS ed_case_ind
	...
	,CASE 
		WHEN intermediate_temp_endoscopy_case.patient_id IS NULL
			THEN NULL
		ELSE 1
		END AS endoscopy_case_ind
	,CASE 
		WHEN intermediate_temp_srgl_case.patient_id IS NULL
			THEN NULL
		ELSE 1
		END AS srgl_case_ind
	,CASE 
		WHEN intermediate_temp_lithotripsy_case.patient_id IS NULL
			THEN NULL
		ELSE 1
		END AS lithotripsy_case_ind
	,CASE 
		WHEN intermediate_temp_cathlab_case.patient_id IS NULL
			THEN NULL
		ELSE 1
...
FROM pce_qe16_oper_prd_zoom..cv_patdisch ZOOM
INNER JOIN pce_qe16_slp_prd_dm..intermediate_temp_eligible_encntr_data ZOOM3YRS
...
LEFT JOIN intermediate_temp_dschrg_inpatient ON intermediate_temp_dschrg_inpatient.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_physician_npi_spclty ATTENDPRACTSPEC ON ZOOM.company_id = ATTENDPRACTSPEC.company_id
	AND ZOOM.attending_practitioner_code = ATTENDPRACTSPEC.practitioner_code
LEFT JOIN intermediate_temp_physician_npi_spclty ADMITPRACTSPEC ON ZOOM.company_id = ADMITPRACTSPEC.company_id
	AND ZOOM.admitting_practitioner_code = ADMITPRACTSPEC.practitioner_code
LEFT JOIN intermediate_temp_dschrg_inpatient_hospice ON intermediate_temp_dschrg_inpatient_hospice.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient_hospice.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_obsrv ON intermediate_temp_obsrv.patient_id = ZOOM.patient_id
	AND intermediate_temp_obsrv.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_icu ON intermediate_temp_icu.patient_id = ZOOM.patient_id
	AND intermediate_temp_icu.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_ccu ON intermediate_temp_ccu.patient_id = ZOOM.patient_id
	AND intermediate_temp_ccu.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_nrs ON intermediate_temp_nrs.patient_id = ZOOM.patient_id
	AND intermediate_temp_nrs.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_rtne ON intermediate_temp_rtne.patient_id = ZOOM.patient_id
	AND intermediate_temp_rtne.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_ed_case ON intermediate_temp_ed_case.patient_id = ZOOM.patient_id
	AND intermediate_temp_ed_case.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_dschrg_inpatient_nbrn ON intermediate_temp_dschrg_inpatient_nbrn.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient_nbrn.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_dschrg_inpatient_rehab ON intermediate_temp_dschrg_inpatient_rehab.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient_rehab.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_dschrg_inpatient_psych ON intermediate_temp_dschrg_inpatient_psych.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient_psych.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_dschrg_inpatient_ltcsnf ON intermediate_temp_dschrg_inpatient_ltcsnf.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient_ltcsnf.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_dschrg_inpatient_spclcare ON intermediate_temp_dschrg_inpatient_spclcare.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient_spclcare.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_dschrg_inpatient_lipmip ON intermediate_temp_dschrg_inpatient_lipmip.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient_lipmip.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_dschrg_inpatient_acute ON intermediate_temp_dschrg_inpatient_acute.patient_id = ZOOM.patient_id
	AND intermediate_temp_dschrg_inpatient_acute.company_id = ZOOM.company_id
--Code Change: Commenting the following since ptnt_days_total would be based on SPL dimension
--LEFT JOIN intermediate_temp_derived_ptnt_days ON intermediate_temp_derived_ptnt_days.patient_id = ZOOM.patient_id
--AND intermediate_temp_derived_ptnt_days.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_srgl_case ON intermediate_temp_srgl_case.patient_id = ZOOM.patient_id 
	AND intermediate_temp_srgl_case.company_id = ZOOM.company_id
--Code Change : To add LITHOTRIPSY, Endoscopy and Cath Lab Case INDICATOR
LEFT JOIN intermediate_temp_cathlab_case ON intermediate_temp_cathlab_case.patient_id = ZOOM.patient_id 
	AND intermediate_temp_cathlab_case.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_lithotripsy_case ON intermediate_temp_lithotripsy_case.patient_id = ZOOM.patient_id 
	AND intermediate_temp_lithotripsy_case.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_endoscopy_case ON intermediate_temp_endoscopy_case.patient_id = ZOOM.patient_id 
	AND intermediate_temp_endoscopy_case.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_derived_ptnt_days_nbrn ON intermediate_temp_derived_ptnt_days_nbrn.patient_id = ZOOM.patient_id
	AND intermediate_temp_derived_ptnt_days_nbrn.company_id = ZOOM.company_id
--Code Change : To add Ptnt_Days_stepdown
LEFT JOIN intermediate_temp_derived_ptnt_days_stepdown ON intermediate_temp_derived_ptnt_days_stepdown.patient_id = ZOOM.patient_id
	AND intermediate_temp_derived_ptnt_days_stepdown.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_derived_ptnt_days_rehab ON intermediate_temp_derived_ptnt_days_rehab.patient_id = ZOOM.patient_id
	AND intermediate_temp_derived_ptnt_days_rehab.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_derived_ptnt_days_acute ON intermediate_temp_derived_ptnt_days_acute.patient_id = ZOOM.patient_id
	AND intermediate_temp_derived_ptnt_days_acute.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_derived_ptnt_days_psych ON intermediate_temp_derived_ptnt_days_psych.patient_id = ZOOM.patient_id
	AND intermediate_temp_derived_ptnt_days_psych.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_payer_fcy_std_code PRIMPAYER ON PRIMPAYER.company_id = ZOOM.company_id
	AND PRIMPAYER.fcy_payer_code = ZOOM.primary_payer_code
LEFT JOIN intermediate_temp_payer_fcy_std_code SECONPAYER ON SECONPAYER.company_id = ZOOM.company_id
	AND SECONPAYER.fcy_payer_code = ZOOM.secondary_payer_code
LEFT JOIN intermediate_temp_payer_fcy_std_code TRTYPAYER ON TRTYPAYER.company_id = ZOOM.company_id
	AND TRTYPAYER.fcy_payer_code = ZOOM.tertiarypayorplan
LEFT JOIN intermediate_temp_payer_fcy_std_code QTRPAYER ON QTRPAYER.company_id = ZOOM.company_id
...
LEFT JOIN intermediate_temp_discharge_fcy_std_status_code DISSTATUS ON DISSTATUS.discharge_status = ZOOM.discharge_status
LEFT JOIN pce_qe16_slp_prd_dm..stnd_ptnt_zip_dim ZIPCODE ON ZIPCODE.ptnt_zip_cd = substr(ZOOM.residential_zip_code, 1, 5)
LEFT JOIN intermediate_temp_ptnt_type_fcy_std_cd PATTYPE ON PATTYPE.patient_type_code = ZOOM.patient_type
	AND PATTYPE.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_ptnt_prim_dgns PRIMDGNS ON PRIMDGNS.patient_id = ZOOM.patient_id
	AND PRIMDGNS.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_ptnt_second_dgns SCDYDGNS ON SCDYDGNS.patient_id = ZOOM.patient_id
	AND SCDYDGNS.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_ptnt_trty_dgns TRTYDGNS ON TRTYDGNS.patient_id = ZOOM.patient_id
	AND TRTYDGNS.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_ptnt_prim_proc PRIMPROC ON PRIMPROC.patient_id = ZOOM.patient_id
	AND PRIMPROC.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_ptnt_scdy_proc SCDYPROC ON SCDYPROC.patient_id = ZOOM.patient_id
	AND SCDYPROC.company_id = ZOOM.company_id
LEFT JOIN intermediate_temp_ptnt_trty_proc TRTYPROC ON TRTYPROC.patient_id = ZOOM.patient_id
...
LEFT JOIN intermediate_temp_encntr_svc_hier SVCRNK
on SVCRNK.company_id = ZOOM.company_id and SVCRNK.patient_id = ZOOM.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_temp_surgeon_pract SURGEON
...
LEFT JOIN  pce_qe16_slp_prd_dm..intermediate_temp_specl_valid_ind SPECLVALID
on SPECLVALID.src_company_id = ZOOM.company_id and ZOOM.patient_id = SPECLVALID.src_patient_id 
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_temp_encntr_dgns_fct_with_cancer_case CANCER
...
on CVDSCHRGSVC.code = ZOOM.dischservice AND CVDSCHRGSVC.facility = ZOOM.company_id
LEFT JOIN pce_qe16_oper_oper_prd_zoom..cv_financialclass CVFNCCLS
