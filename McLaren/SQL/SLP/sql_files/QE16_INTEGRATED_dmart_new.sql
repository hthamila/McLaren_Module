...
DROP TABLE temp_eligible_encntr_data IF EXISTS;
	CREATE TEMP TABLE temp_eligible_encntr_data AS (
		...
		ZOOM.admission_ts, ZOOM.discharge_ts
		,ZOOM.msdrg_code 
		...
		,CASE WHEN Date(zoom.discharge_ts) BETWEEN  (Select Fiscal_start from temp_fiscal_year_tbl) and now()- Day(now()) THEN 'C'
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (select DATE(fiscal_start) -1 FROM temp_fiscal_year_tbl) THEN 'P'
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND Date(zoom.admission_ts) BETWEEN  (Select Fiscal_start from temp_fiscal_year_tbl) and now()-DAY(now())  THEN 'C'
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (select DATE(fiscal_start) -1 FROM temp_fiscal_year_tbl)  THEN 'P'
...
--			   ELSE 
--			   NULL END as fiscal_yr
		FROM pce_qe16_oper_prd_zoom..cv_patdisch ZOOM WHERE
 coalesce(ZOOM.msdrg_code,'000') != 'V45' AND 
...
( (cast(ZOOM.admission_ts AS DATE) BETWEEN DATE('2015-10-01') AND DATE(now()- Day(now()))) OR (cast(ZOOM.discharge_ts AS DATE) BETWEEN DATE('2015-10-01') AND DATE(now()- Day(now())))) 
...
,coalesce(NPIREG.pvdr_lgl_last_nm ||  ', ' || NPIREG.pvdr_frst_nm, NPIREG.pvdr_lgl_org_nm)   as practitioner_name
         --       ,NPIREG.pvdr_lgl_last_nm ||  ', ' || NPIREG.pvdr_frst_nm  as practitioner_name
		,coalesce(CWALK.pvdr_spclty_descr , NPIREG.mcare_spcly_descr)  AS practitioner_spclty_description 
...
--drop table svc_hier_dim if exists; 
--create table svc_hier_dim as sElect * from svc_hier_dim; 

--QADV Table creation based on Net 3 years Of patient Account Number
DROP TABLE stage_encntr_qly_anl_fct IF EXISTS;

CREATE TABLE stage_encntr_qly_anl_fct as
...
--stage_chrg_fct Table creation based on Net 3 years Of patient Account Number
DROP TABLE pce_qe16_slp_prd_dm..stage_chrg_fct_temp IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..stage_chrg_fct_temp AS 
(
  SELECT Z.company_id as src_company_id,
       ...
       ,CH.raw_chargcode
	   ,Z.fiscal_yr
	   ,nvl(CH.total_charge * RCC.directrcc,  0) as rcc_based_direct_cst_amt
	   ,nvl(CH.total_charge * RCC.indirectrcc, 0) as rcc_based_indirect_cst_amt
	   ,nvl(CH.total_charge * RCC.totalrcc, 0) as rcc_based_total_cst_amt
...
DROP TABLE stage_chrg_fct IF EXISTS;
CREATE TABLE stage_chrg_fct as 
SELECT
hash8(stage_chrg_fct_temp.src_company_id)::bigint as src_company_id_hash
,hash8(stage_chrg_fct_temp.src_patient_id)::bigint as src_patient_id_hash
,hash8(stage_chrg_fct_temp.charge_code)::bigint as charge_code_hash
,stage_chrg_fct_temp.*
--,spl_dim.cdm_cd
--,spl_dim.cdm_strt_cdr_dk
--,spl_dim.cdm_strt_dt
--,spl_dim.cdm_end_cdr_dk
--,spl_dim.cdm_end_dt
,spl_dim.cdm_descr
,spl_dim.persp_clncl_dtl_pcd_cd_v10
,spl_dim.persp_clncl_dtl_pcd_descr_v10
,spl_dim.spl_unit_cnvr
,spl_dim.persp_clncl_dtl_cd
,spl_dim.persp_clncl_dtl_descr
,spl_dim.persp_clncl_dtl_unit
,spl_dim.persp_clncl_smy_cd
,spl_dim.persp_clncl_smy_descr
,spl_dim.persp_clncl_std_dept_cd_v10
,spl_dim.persp_clncl_std_dept_descr_v10
,spl_dim.persp_clncl_std_dept_v10_rollup_cgy_cd
,spl_dim.persp_clncl_std_dept_v10_rollup_cgy_descr
,spl_dim.persp_clncl_dtl_spl_modfr_cd
,spl_dim.persp_clncl_dtl_spl_modfr_descr
...
,row_number() over(partition by stage_chrg_fct_temp.src_company_id, stage_chrg_fct_temp.src_patient_id
Order by  stage_chrg_fct_temp.service_date) as rec_num
FROM stage_chrg_fct_temp
LEFT JOIN prmretlp.spl_dim on stage_chrg_fct_temp.charge_code=spl_dim.cdm_cd and stage_chrg_fct_temp.fcy_num=spl_dim.fcy_num
LEFT JOIN prmretlp.rev_cl_dim on stage_chrg_fct_temp.revenue_code = rev_cl_dim.rev_cd 
LEFT JOIN prmretlp.hcpcs_dim  on stage_chrg_fct_temp.cpt_code = hcpcs_dim.hcpcs_cd
LEFT JOIN prmretlp.dept_dim on  stage_chrg_fct_temp.company_id = dept_dim.company_id and stage_chrg_fct_temp.dept = dept_dim.department_code
...
DROP TABLE pce_qe16_slp_prd_dm..stage_chrg_cost_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..stage_chrg_cost_fct AS 
(
select 
  src_company_id
, src_patient_id 
, ROUND(sum(rcc_based_direct_cst_amt), 4)   as  agg_rcc_based_direct_cst_amt
, ROUND(sum(rcc_based_indirect_cst_amt), 4) as agg_rcc_based_indirect_cst_amt
, ROUND(sum(rcc_based_total_cst_amt), 4)      as agg_rcc_based_total_cst_amt
FROM pce_qe16_slp_prd_dm..stage_chrg_fct
GROUP BY 1,2
);

--select 'Total in stage_chrg_cost_fct' , count(*) from  pce_qe16_slp_prd_dm..stage_chrg_cost_fct;
--stage_encntr_pract_fct Table  creation based on Net 3 years Of patient Account Number


DROP TABLE pce_qe16_slp_prd_dm..stage_encntr_pract_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..stage_encntr_pract_fct AS 
(
  SELECT 
...
--stage_cpt_fct tABLE 
DROP TABLE pce_qe16_slp_prd_dm..stage_cpt_fct IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..stage_cpt_fct AS 
SELECT 
hash8(Z.company_id) as src_company_id_hash
,hash8(Z.patient_id) as src_patient_id_hash
,Z.company_id as src_company_id
...
--stage_encntr_dgns_fct Table  creation based on Net 3 years Of patient Account Number
DROP TABLE pce_qe16_slp_prd_dm..stage_encntr_dgns_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..stage_encntr_dgns_fct AS 
SELECT 
		hash8(Z.company_id) as src_company_id_hash
	   ,hash8(Z.patient_id) as src_patient_id_hash
	   ,Z.company_id as src_company_id
...
-----stage_encntr_pcd_fct Table  creation based on Net 3 years Of patient Account Number
DROP TABLE pce_qe16_slp_prd_dm..stage_encntr_pcd_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..stage_encntr_pcd_fct AS 
SELECT 
		hash8(Z.company_id) as src_company_id_hash
	   , hash8(Z.patient_id) as src_patient_id_hash
	   , Z.company_id as src_company_id
...
--stage_svc_ln_anl_fct Code
--based on cv_patbill / stage_chrg_fct 
...
EN.company_id, EN.patient_id ,EN.inpatient_outpatient_flag, 'hcpcs/cpt' as based_on,
--nvl(RNK.cd,'-100'),
...
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
from temp_eligible_encntr_data EN
LEFT JOIN pce_qe16_slp_prd_dm..stage_encntr_pcd_fct X 
...
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
from temp_eligible_encntr_data EN
LEFT JOIN pce_qe16_slp_prd_dm..stage_cpt_fct X
on EN.company_id = X.company_id and EN.patient_id = X.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..svc_hier_dim RNK
...
DROP TABLE stage_svc_ln_anl_fct IF exists;
CREATE TABLE stage_svc_ln_anl_fct As 
with  all_joined as (
...
from temp_eligible_encntr_data Z
LEFT JOIN pce_qe16_slp_prd_dm..stage_svc_ln_anl_fct SVCLN
on Z.company_id = SVCLN.company_id and Z.patient_id = SVCLN.patient_id and SVCLN.svc_ln is NOT NULL
LEFT JOIN pce_qe16_slp_prd_dm..svc_hier_dim HIER
on HIER.cd = SVCLN.cd
...
IF EXISTS;
	CREATE TEMP TABLE temp_ptnt_adm_dgns AS (
		...
		FROM pce_qe16_slp_prd_dm..stage_encntr_dgns_fct X
...
IF EXISTS;
	CREATE TEMP TABLE temp_ptnt_prim_proc AS (
		...
		FROM pce_qe16_slp_prd_dm..stage_encntr_pcd_fct X
		INNER JOIN temp_eligible_encntr_data ENCNTR ON ENCNTR.company_id = X.company_id
			AND ENCNTR.patient_id = X.patient_id
...
IF EXISTS;
	CREATE TEMP TABLE temp_obsrv AS (
		...
		,pb.company_id
		,sum(pb.quantity) AS qty FROM pce_qe16_slp_prd_dm..stage_chrg_fct pb WHERE pb.revenue_code = '0762' GROUP BY pb.patient_id
		,pb.company_id
		);--215561
...
IF EXISTS;
	CREATE TEMP TABLE temp_icu AS (
		...
		,ZOOM.company_id
		,CASE WHEN(sum(DISTINCT ZOOM.quantity) =0 ) THEN NULL ELSE sum(DISTINCT ZOOM.quantity) END AS icu_days
		FROM pce_qe16_slp_prd_dm..stage_chrg_fct ZOOM 
--		 INNER JOIN temp_dschrg_inpatient ON temp_dschrg_inpatient.patient_id = ZOOM.patient_id 
		 INNER JOIN pce_qe16_slp_prd_dm..spl_dim SP 
...
IF EXISTS;
	CREATE TEMP TABLE temp_ccu AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
		,CASE WHEN(sum(DISTINCT ZOOM.quantity) =0 ) THEN NULL ELSE sum(DISTINCT ZOOM.quantity) END AS ccu_days
		FROM pce_qe16_slp_prd_dm..stage_chrg_fct ZOOM 
--		 INNER JOIN temp_dschrg_inpatient ON temp_dschrg_inpatient.patient_id = ZOOM.patient_id 
		 INNER JOIN pce_qe16_slp_prd_dm..spl_dim SP 
		 ON SP.fcy_num = ZOOM.fcy_num and ZOOM.charge_code = SP.cdm_cd
...
IF EXISTS;
	CREATE TEMP TABLE temp_nrs AS (
		SELECT patient_id
		,company_id
		,count(DISTINCT pb.service_date) AS nrs_days FROM pce_qe16_slp_prd_dm..stage_chrg_fct pb WHERE pb.revenue_code BETWEEN '0170'
...
IF EXISTS;
	CREATE TEMP TABLE temp_rtne AS (
		SELECT patient_id
		,company_id
		,count(DISTINCT pb.service_date) AS rtne_days FROM pce_qe16_slp_prd_dm..stage_chrg_fct pb WHERE (
			(
...
IF EXISTS;
	CREATE TEMP TABLE temp_ed_case AS (
		...
		,company_id FROM pce_qe16_slp_prd_dm..stage_chrg_fct PB INNER JOIN pce_qe16_slp_prd_dm..val_set_dim VSET_ED_CPT ON VSET_ED_CPT.cd = PB.cpt_code
...
IF EXISTS;
	CREATE TEMP TABLE temp_dschrg_inpatient_rehab AS (
		...
		AND Z.inpatient_outpatient_flag = 'I'
		AND (
			lower(admitservice) = lower('Rehab')
			OR lower(dischargeservice) = lower('Rehab')
...
IF EXISTS;
	CREATE TEMP TABLE temp_derived_ptnt_days AS (
		...
		FROM pce_qe16_slp_prd_dm..stage_chrg_fct ZOOM INNER JOIN temp_dschrg_inpatient ON temp_dschrg_inpatient.patient_id = ZOOM.patient_id WHERE (
			(
...
IF EXISTS;
	CREATE TEMP TABLE temp_endoscopy_case AS (
		SELECT DISTINCT patient_id
		,company_id FROM pce_qe16_slp_prd_dm..stage_chrg_fct WHERE revenue_code IN (
			'0750'
			) GROUP BY patient_id
		,company_id
		);
...
IF EXISTS;
	CREATE TEMP TABLE temp_srgl_case AS (
		SELECT DISTINCT patient_id
		,company_id FROM pce_qe16_slp_prd_dm..stage_chrg_fct CF 
		INNER JOIN pce_qe16_slp_prd_dm..spl_dim SP
...
--code change : Added logic to calculate  Lithotripsy  Cases based on SPL Dimension
 ...
 --code change : Added logic to calculate  CathLab Cases based on stage_svc_ln_anl_fct
    DROP TABLE temp_cathlab_case IF EXISTS;
	CREATE TEMP TABLE temp_cathlab_case AS
	(
	  SELECT DISTINCT patient_id, company_id 
	  FROM pce_qe16_slp_prd_dm..stage_svc_ln_anl_fct 
...
--New
DROP TABLE pce_qe16_slp_prd_dm..stage_encntr_cnslt_pract_fct IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..stage_encntr_cnslt_pract_fct as 
...
FROM temp_eligible_encntr_data Z
INNER JOIN pce_qe16_slp_prd_dm..stage_encntr_pract_fct C1
...
(
  ...
  from temp_eligible_encntr_data Z
  LEFT JOIN pce_qe16_slp_prd_dm..stage_encntr_pcd_fct P 
...
(
  ...
  case when ROUND((X.accountbalance/X.discharge_total_charges * 100),4) <= 10 THEN
      'Y'
	  ELSE 
	   'N' END as est_acct_paid_ind,
  ROUND((X.accountbalance/X.discharge_total_charges),4) as acct_bal_pcnt,
...
ROUND(tot_pymt_amt/tot_chrg_amt,4) as pymt_ratio
FROM 
(
	select  
	   in_or_out_patient_ind ,
	   ...
	   ROUND(sum(tot_pymt_amt ) ,4) as  tot_pymt_amt,
       ROUND(sum(tot_chrg_amt),4) as tot_chrg_amt
FROM temp_table_all_ip_rows Z
WHERE est_acct_paid_ind ='Y' and src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid') and 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) 
...
select ms_drg_cd, CAST(case_mix_idnx_num as NUMERIC(14,4)) as drg_wght
...
ROUND(X.tot_pymt_amt/X.tot_chrg_amt,4) as pymt_ratio,
...
UNION 
select UNPD.*, 
DRGWGHT.drg_wght * PAID.drg_weighted_pmnt_per_case as est_net_rev_amt
...
on UNPD.ms_drg_cd  = PAID.ms_drg_cd and UNPD.src_prim_payor_grp1 = PAID.src_prim_payor_grp1
WHERE UNPD.est_acct_paid_ind ='N' and UNPD.src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid')  and (UNPD.ms_drg_cd IS NOT NULL  AND  UNPD.ms_drg_cd !='-100')
and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) 
...
select UNPD.*, 
UNPD.tot_chrg_amt * PAID.pymt_ratio as est_net_rev_amt
from temp_table_all_ip_rows UNPD
LEFT JOIN pce_qe16_slp_prd_dm..ip_hist_pymt_ratio_case_a PAID
on  UNPD.src_prim_payor_grp1 = PAID.src_prim_payor_grp1
WHERE UNPD.est_acct_paid_ind ='N' and UNPD.src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid')  and (UNPD.ms_drg_cd IS NULL  OR UNPD.ms_drg_cd = '-100')
and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) ;
...
(
	select  
       ...
       ROUND(sum(tot_chrg_amt),4) as tot_chrg_amt
FROM temp_table_all_ip_rows Z
WHERE est_acct_paid_ind ='Y' and src_prim_payor_grp1 in ('Other') and 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) 
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30')
Group by 1,2) X;
...
on UNPD.ms_drg_cd  = PAID.ms_drg_cd and UNPD.src_prim_payor_grp1 = PAID.src_prim_payor_grp1
WHERE UNPD.est_acct_paid_ind ='N' and UNPD.src_prim_payor_grp1 in ('Other')   and (UNPD.ms_drg_cd IS NOT NULL  AND  UNPD.ms_drg_cd !='-100')
and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) 
and dschrg_dt BETWEEN DRGWGHT.vld_fm_dt AND DRGWGHT.vld_to_dt
...
on  UNPD.src_prim_payor_grp1 = PAID.src_prim_payor_grp1
WHERE UNPD.est_acct_paid_ind ='N' and UNPD.src_prim_payor_grp1 in ('Other')  and (UNPD.ms_drg_cd IS NULL  OR UNPD.ms_drg_cd = '-100')
and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) ;
...
select ALLCASES.*, 
ALLCASES.tot_chrg_amt * RATIO.pymt_to_chrg_ratio as est_net_rev_amt
...
AND RATIO.ptnt_cgy= 'Inpatient' and RATIO.payor_group_1 = 'Domestic' AND 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) ;
--AND cast(dschrg_dt AS DATE) BETWEEN DATE ('2017-10-01') AND  DATE ('2018-09-30');
;
...
SELECT
  ...
  ROUND((ALLCASES.acct_bal_amt/ALLCASES.tot_chrg_amt),4) as acct_bal_pcnt,
  ALLCASES.src_prim_payor_grp1,
  1 as cnt,
  CASE WHEN ALLCASES.acct_bal_amt = 0 THEN 
      ALLCASES.tot_pymt_amt 
	ELSE
	   ALLCASES.tot_chrg_amt * RATIO.pymt_to_chrg_ratio
    END AS  est_net_rev_amt
FROM temp_table_all_ip_rows ALLCASES
LEFT JOIN pce_qe16_slp_prd_dm..manual_pymt_chrg_ratio RATIO
on RATIO.payor_group_1  = ALLCASES.src_prim_payor_grp1 AND RATIO.company_id = ALLCASES.fcy_nm
WHERE ALLCASES.src_prim_payor_grp1 in ('Self Pay' ) and 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) 
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30')
...
on UNPD.ptnt_tp_cd  = PAID.ptnt_tp_cd and UNPD.src_prim_payor_grp1 = PAID.src_prim_payor_grp1
WHERE UNPD.est_acct_paid_ind ='N' and UNPD.src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid') and 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) 
--cast(dschrg_dt AS DATE) BETWEEN DATE ('2017-10-01') AND  DATE ('2018-09-30')
UNION 
--Paid Cases 
...
WHERE PAID.est_acct_paid_ind ='Y' and PAID.src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid') and 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) ;
...
on UNPD.ptnt_tp_cd  = PAID.ptnt_tp_cd and UNPD.src_prim_payor_grp1 = PAID.src_prim_payor_grp1
WHERE UNPD.est_acct_paid_ind ='N' and UNPD.src_prim_payor_grp1 in ('Other') and 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) 
--cast(dschrg_dt AS DATE) BETWEEN DATE ('2017-10-01') AND  DATE ('2018-09-30')
UNION 
select PAID.*, 
...
PAID.tot_pymt_amt +  PAID.acct_bal_amt as est_net_rev_amt
FROM temp_table_all_op_rows PAID
WHERE PAID.est_acct_paid_ind ='Y' and PAID.src_prim_payor_grp1 in ('Other') and 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) ;
--cast(dschrg_dt AS DATE) BETWEEN DATE ('2017-10-01') AND  DATE ('2018-09-30');
...
on RATIO.payor_group_1  = ALLCASES.src_prim_payor_grp1 AND RATIO.company_id = ALLCASES.fcy_nm
WHERE ALLCASES.src_prim_payor_grp1 in ('Domestic')  
AND RATIO.ptnt_cgy= 'Outpatient' and RATIO.payor_group_1 = 'Domestic' AND 
cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) ;
--AND cast(dschrg_dt AS DATE) BETWEEN DATE ('2017-10-01') AND  DATE ('2018-09-30');
;
...
select fcy_nm,
ROUND(sum(tot_chrg_amt), 4) as grs_rev_amt , 
ROUND(sum(est_net_rev_amt), 4) as drvd_net_rev_amt 
...
INNER JOIN pce_qe16_slp_prd_dm..stage_chrg_fct C
...
DROP TABLE pce_qe16_slp_prd_dm..stage_encntr_net_rvu_fct_x IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..stage_encntr_net_rvu_fct_x AS
with combined as 
...
DROP TABLE pce_qe16_slp_prd_dm..stage_encntr_net_rvu_fct IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..stage_encntr_net_rvu_fct AS
with combined as 
(select * from pce_qe16_slp_prd_dm..op_encntr_net_rvu
UNION 
select * from pce_qe16_slp_prd_dm..ip_encntr_net_rvu),
...
,X.patient_id as encntr_num 
,Y.est_acct_paid_ind
,ROUND(Y.est_net_rev_amt, 4) as est_net_rev_amt
,nvl(Z.prof_chrg_ind, 'N') as prof_chrg_ind
--Y.*
FROM temp_eligible_encntr_data X
LEFT JOIN combined Y
...
DROP TABLE pce_qe16_slp_prd_dm..stage_hist_pymt_ratio IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..stage_hist_pymt_ratio as 
...
DROP TABLE pce_qe16_slp_prd_dm..stage_hist_pymt_ratio_drg_wghts IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..stage_hist_pymt_ratio_drg_wghts as 
...
DROP TABLE pce_qe16_slp_prd_dm..stage_net_rvu_model IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..stage_net_rvu_model as 
...
DROP TABLE pce_qe16_slp_prd_dm..stage_encntr_anl_fct IF EXISTS;

	CREATE TABLE pce_qe16_slp_prd_dm..stage_encntr_anl_fct AS
...
SELECT Distinct
	...
	,ZOOM.discharge_ts AS dschrg_ts
...
--Code Change : Zoom gets from Encntr but Integrated Mart gets from stage_encntr_dgns_fct so commenting Integrated Version
...
--Code Change: Zoom gets from Encntr but Integrated Mart gets from stage_encntr_pcd_fct so commenting Integrated Version
...
--	,nvl(PRIMPROC.prim_proc_icd_pcd_descr,'UNKNOWN') as prim_pcd_descr
	...
	,CASE WHEN ZOOM.inpatient_outpatient_flag = 'O' THEN 
		...
		WHEN ZOOM.inpatient_outpatient_flag = 'I'  AND MSDRGDIM.ms_drg_type_cd = 'SURG'     THEN 
	        SURGEON.prim_srgn_cd
		WHEN ZOOM.inpatient_outpatient_flag = 'I'  AND MSDRGDIM.ms_drg_type_cd = 'OTH'     THEN 
	        nvl(ATTENDPRACTSPEC.practitioner_code,'-100') 
...
LEFT JOIN pce_qe16_slp_prd_dm..stage_chrg_cost_fct CHRGRCC
on ZOOM.company_id = CHRGRCC.src_company_id and ZOOM.patient_id = CHRGRCC.src_patient_id
LEFT JOIN pce_qe16_slp_prd_dm..stage_encntr_qly_anl_fct QADV ON ZOOM.patient_id = QADV.encntr_num --AND ZOOM.company_id = QADV.fcy_num 
...
LEFT JOIN pce_qe16_slp_prd_dm..dgns_dim DGNSDIM ON DGNSDIM.dgns_cd = ZOOM.primaryicd10diagnosiscode and DGNSDIM.dgns_icd_ver ='ICD10' 
LEFT JOIN pce_qe16_slp_prd_dm..dgns_dim ADMDGNS ON ADMDGNS.dgns_cd = ZOOM.admitdiagnosiscode
...
LEFT JOIN pce_qe16_slp_prd_dm..stage_encntr_cnslt_pract_fct CNSLT
on CNSLT.src_company_id = ZOOM.company_id and CNSLT.src_patient_id = ZOOM.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..stage_encntr_net_rvu_fct NETREV
...
WHERE coalesce(ZOOM.msdrg_code ,'000') <> 'V45'
