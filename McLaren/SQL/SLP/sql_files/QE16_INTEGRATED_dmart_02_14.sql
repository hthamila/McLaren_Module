...
select 
  src_company_id
, src_patient_id 
, ROUND(sum(rcc_based_direct_cst_amt), 2)   as  agg_rcc_based_direct_cst_amt
, ROUND(sum(rcc_based_indirect_cst_amt), 2) as agg_rcc_based_indirect_cst_amt
, ROUND(sum(rcc_based_total_cst_amt), 2)      as agg_rcc_based_total_cst_amt
...
,coalesce(NPIREG.pvdr_lgl_last_nm ||  ', ' || NPIREG.pvdr_frst_nm, PT.practitioner_name)   as practitioner_name
...
from pce_qe16_oper_prd_zoom..cv_patdisch X 
 ...
 on Z.company_id = X.company_id and Z.patient_id = X.patient_id
  WHERE 
  X.inpatient_outpatient_flag = 'I' and X.discharge_total_charges > 0 AND X.company_id !='Lansing' and 
  --Added Ptnt_tp_Cd Exclusions based on "Derived Net Revenue Reference Documents"
  upper(X.patient_type) NOT in ('LIP','MIP','BSCH','BSCHO','8','C','F','GCLK','LLOV','MCIV','OFCE','OFFICE','OFFICE SERIES','POV','PRO','Z','ZWH')
)
...
from pce_qe16_oper_prd_zoom..cv_patdisch X 
 ...
 on Z.company_id = X.company_id and Z.patient_id = X.patient_id
  WHERE 
    X.inpatient_outpatient_flag = 'O' and X.discharge_total_charges > 0 AND X.company_id !='Lansing' ANd 
  --Added Ptnt_tp_Cd Exclusions based on "Derived Net Revenue Reference Documents"
  upper(X.patient_type) NOT in ('LIP','MIP','BSCH','BSCHO','8','C','F','GCLK','LLOV','MCIV','OFCE','OFFICE','OFFICE SERIES','POV','PRO','Z','ZWH')
)
