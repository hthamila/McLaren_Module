...
DROP TABLE temp_eligible_encntr_data IF EXISTS;
	CREATE TEMP TABLE temp_eligible_encntr_data AS (
		...
		,CASE WHEN Date(zoom.discharge_ts) BETWEEN  (Select Fiscal_start from temp_fiscal_year_tbl) and now()- Day(now()) THEN 'C'
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(Now())) - Interval '1 Year' THEN 'P'
				      ...
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from temp_fiscal_year_tbl) and (now()- Day(now())) - Interval '1 Year' THEN 'P'
...
--			   NULL END as fiscal_yr
 ...
 coalesce(ZOOM.msdrg_code,'000') != 'V45' AND initcap(ZOOM.company_id) <> 'Lansing' AND 
...
select Z.company_id as src_company_id, Z.patient_id as src_patient_id , QADV.* from temp_eligible_encntr_data Z
...
DROP TABLE pce_qe16_slp_prd_dm..stage_chrg_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..stage_chrg_fct AS 
(
...
,coalesce(NPIREG.pvdr_lgl_last_nm ||  ', ' || NPIREG.pvdr_frst_nm, NPIREG.pvdr_lgl_org_nm)   as practitioner_name
		...
		,NPIREG.prim_spcly_nm AS practitioner_spclty_description 
...
(
  SELECT Z.company_id as src_company_id, Z.patient_id as src_patient_id
       ,CH.company_id
       ,CH.patient_id
...
select  EN.company_id, EN.patient_id ,EN.inpatient_outpatient_flag, 'hcpcs/cpt' as based_on,
--nvl(RNK.cd,'-100'),
...
select  EN.company_id, EN.patient_id ,EN.inpatient_outpatient_flag, 'pcs' as  based_on,
--nvl(RNK.cd,'-100'),
RNK.svc_cgy,RNK.svc_ln,RNK.sub_svc_ln , RNK.services as svc_nm, RNK.cd, RNK.cd_type,RNK.descr as cd_descr,
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
from temp_eligible_encntr_data EN
...
select  EN.company_id, EN.patient_id ,EN.inpatient_outpatient_flag, 'cpt' as based_on,
--nvl(RNK.cd,'-100'),
RNK.svc_cgy,RNK.svc_ln,RNK.sub_svc_ln , RNK.services as svc_nm, RNK.cd, RNK.cd_type,RNK.descr as cd_descr,
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
from temp_eligible_encntr_data EN
LEFT JOIN pce_qe16_slp_prd_dm..cpt_fct X
on EN.company_id = X.company_id and EN.patient_id = X.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..svc_hier_dim RNK
...
select  EN.company_id, EN.patient_id ,EN.inpatient_outpatient_flag, 'msdrg' as based_on,
--nvl(RNK.cd,'-100'),
RNK.svc_cgy,RNK.svc_ln,RNK.sub_svc_ln , RNK.services as svc_nm, RNK.cd, RNK.cd_type,RNK.descr as cd_descr,
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
...
from pce_qe16_oper_prd_zoom..cv_patdisch X 
 ...
 on Z.company_id = X.company_id and Z.patient_id = X.patient_id
  WHERE 
    X.inpatient_outpatient_flag = 'O' and X.discharge_total_charges > 0 AND X.company_id !='Lansing'
  --Added Ptnt_tp_Cd Exclusions based on "Derived Net Revenue Reference Documents"
