...
DROP TABLE intermediate_staging_temp_fiscal_year_tbl IF EXISTS; 
CREATE Table intermediate_staging_temp_fiscal_year_tbl AS 
...
DROP TABLE intermediate_staging_temp_dates_tbl IF EXISTS; 

CREATE TEMP TABLE intermediate_staging_temp_dates_tbl 
AS 
select 
...
DROP TABLE intermediate_staging_temp_eligible_encntr_data_inpatient IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_eligible_encntr_data_inpatient AS (
		...
		,CASE WHEN Date(zoom.discharge_ts) BETWEEN  (Select Fiscal_start from intermediate_staging_temp_fiscal_year_tbl) and --now()- Day(now()) THEN 'C'
		 (select curr_year_end_dt from intermediate_staging_temp_dates_tbl) THEN 'C' 
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '1 year' FROM intermediate_staging_temp_dates_tbl) THEN 'P'
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '2 year' from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '2 year' FROM intermediate_staging_temp_dates_tbl) THEN 'P-1'
					  WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND Date(zoom.admission_ts) BETWEEN  (Select Fiscal_start from intermediate_staging_temp_fiscal_year_tbl) and 
                       (select curr_year_end_dt	FROM intermediate_staging_temp_dates_tbl)				  THEN 'C'
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '1 year' FROM intermediate_staging_temp_dates_tbl)  THEN 'P'
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '2 year' from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '2 year' FROM intermediate_staging_temp_dates_tbl)  THEN 'P-1'                       
...
SELECT count(*)
FROM intermediate_staging_temp_eligible_encntr_data_inpatient;

DROP TABLE intermediate_staging_temp_eligible_encntr_data_outpatient IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_eligible_encntr_data_outpatient AS (
		SELECT DISTINCT ZOOM.company_id
		,ZOOM.patient_id , ZOOM.inpatient_outpatient_flag ,
		ZOOM.admission_ts
		,case WHEN ZOOM.inpatient_outpatient_flag ='O' THEN COALESCE(Date(zoom.discharge_ts), Date(zoom.admission_ts)) ELSE Date(zoom.discharge_ts) END AS discharge_ts
        ...
        ,CAST(NULL as NUMERIC(14,4)) as ms_drg_arthm_mean_los_num
                ,CASE WHEN  MONTH(date_trunc('quarter',date(discharge_ts))) >= 10  THEN 'FY' || (YEAR(discharge_ts) + 1) 
					   ...
					   ELSE
					   NULL END as fiscal_yr
		,CASE WHEN Date(zoom.discharge_ts) BETWEEN  (Select Fiscal_start from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt from intermediate_staging_temp_dates_tbl) THEN 'C' 
				      WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '1 year' FROM intermediate_staging_temp_dates_tbl) THEN 'P'
	                  WHEN Date(zoom.discharge_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '2 year' from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '2 year' FROM intermediate_staging_temp_dates_tbl) THEN 'P-1'
	WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND Date(zoom.admission_ts) BETWEEN  (Select Fiscal_start from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt	FROM intermediate_staging_temp_dates_tbl) THEN 'C'
				      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '1 year' FROM intermediate_staging_temp_dates_tbl)  THEN 'P'
                      WHEN ZOOM.inpatient_outpatient_flag ='O' AND Date(zoom.discharge_ts) IS NULL AND
                                           Date(zoom.admission_ts) BETWEEN  (Select DATE(Fiscal_start) - Interval '2 year' from intermediate_staging_temp_fiscal_year_tbl) and (select curr_year_end_dt - Interval '2 year' FROM intermediate_staging_temp_dates_tbl)  THEN 'P-1'                                     
...
SELECT count(*)
FROM intermediate_staging_temp_eligible_encntr_data_outpatient;

--CODE CHANGE : AUG 2019 (a) Ms_Drg_Dim Historical CMI Weights
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_ms_drg_dim_hist  IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_ms_drg_dim_hist AS 
select ms_drg_cd, 
...
--CODE CHANGE : AUG 2019 (a) Ms_Drg_Dim Historical CMI Weights   intermediate_staging_temp_eligible_encntr_data
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_eligible_encntrs IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_eligible_encntrs 
AS 
SELECT X.*
FROM intermediate_staging_temp_eligible_encntr_data_inpatient X
UNION 
SELECT Y.*
FROM intermediate_staging_temp_eligible_encntr_data_outpatient Y; 
		
SELECT count(*)
FROM intermediate_staging_temp_eligible_encntrs;

DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_encntr_with_ms_drg_wghts IF EXISTS; 

CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_encntr_with_ms_drg_wghts AS 
SELECT 
...
FROM pce_qe16_slp_prd_dm..intermediate_staging_temp_eligible_encntrs X 
INNER JOIN pce_qe16_slp_prd_dm..intermediate_staging_temp_ms_drg_dim_hist DRGWGHT
ON X.msdrg_code= DRGWGHT.ms_drg_cd  AND date(X.discharge_ts)  BETWEEN DRGWGHT.vld_fm_dt AND DRGWGHT.vld_to_dt;


DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_encntr_without_ms_drg_wghts IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_encntr_without_ms_drg_wghts AS 
with recs_with_weights AS 
(select distinct patient_id , company_id FROM pce_qe16_slp_prd_dm..intermediate_staging_temp_encntr_with_ms_drg_wghts)
select * FROM pce_qe16_slp_prd_dm..intermediate_staging_temp_eligible_encntrs X  
WHERE (patient_id || company_id) NOT IN (select (patient_id || company_id) from recs_with_weights );

DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_eligible_encntr_data IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_eligible_encntr_data 
AS 
SELECT X.*
FROM intermediate_staging_temp_encntr_with_ms_drg_wghts X
UNION 
SELECT Y.*
FROM intermediate_staging_temp_encntr_without_ms_drg_wghts Y; 
		
SELECT count(*)
FROM intermediate_staging_temp_eligible_encntr_data;


----------------------------------------


DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_physician_npi_spclty

IF EXISTS;
	CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_physician_npi_spclty AS (
...
--CODE Change : Adding intermediate_staging_intermediate_staging_spl_dim to fix the Lansing encounters with Charge Code but SPL Code is NULL issue 
DROP TABLE intermediate_staging_spl_dim IF EXISTS;
CREATE TABLE intermediate_staging_spl_dim as
...
DROP TABLE intermediate_staging_encntr_qly_anl_fct IF EXISTS;

CREATE TABLE intermediate_staging_encntr_qly_anl_fct as
select hash8(z.company_id)::bigint as src_company_id_hash,
hash8(Z.patient_id)::bigint as src_patient_id_hash,
Z.company_id as src_company_id, Z.patient_id as src_patient_id , QADV.* from intermediate_staging_temp_eligible_encntr_data Z
...
--intermediate_staging_chrg_fct Table creation based on Net 3 years Of patient Account Number
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct_temp IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct_temp AS 
(
...
--	   ,RAWCHRGCD.charge_code_description as raw_chrg_cd_descr
  FROM intermediate_staging_temp_eligible_encntr_data Z
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct as 
SELECT
hash8(intermediate_staging_chrg_fct_temp.src_company_id)::bigint as src_company_id_hash
,hash8(intermediate_staging_chrg_fct_temp.src_patient_id)::bigint as src_patient_id_hash
,hash8(intermediate_staging_chrg_fct_temp.charge_code)::bigint as charge_code_hash
,intermediate_staging_chrg_fct_temp.*
--,intermediate_staging_spl_dim.cdm_cd
--,intermediate_staging_spl_dim.cdm_strt_cdr_dk
--,intermediate_staging_spl_dim.cdm_strt_dt
--,intermediate_staging_spl_dim.cdm_end_cdr_dk
--,intermediate_staging_spl_dim.cdm_end_dt
--,intermediate_staging_spl_dim.cdm_descr
,intermediate_staging_spl_dim.persp_clncl_dtl_pcd_cd_v10
,intermediate_staging_spl_dim.persp_clncl_dtl_pcd_descr_v10
,intermediate_staging_spl_dim.spl_unit_cnvr
,intermediate_staging_spl_dim.persp_clncl_dtl_cd
,intermediate_staging_spl_dim.persp_clncl_dtl_descr
,intermediate_staging_spl_dim.persp_clncl_dtl_unit
,intermediate_staging_spl_dim.persp_clncl_smy_cd
,intermediate_staging_spl_dim.persp_clncl_smy_descr
,intermediate_staging_spl_dim.persp_clncl_std_dept_cd_v10
,intermediate_staging_spl_dim.persp_clncl_std_dept_descr_v10
,intermediate_staging_spl_dim.persp_clncl_std_dept_v10_rollup_cgy_cd
,intermediate_staging_spl_dim.persp_clncl_std_dept_v10_rollup_cgy_descr
,intermediate_staging_spl_dim.persp_clncl_dtl_spl_modfr_cd
,intermediate_staging_spl_dim.persp_clncl_dtl_spl_modfr_descr
...
,CASE WHEN intermediate_staging_spl_dim.persp_clncl_smy_descr = 'SURGERY TIME' AND 
UPPER(intermediate_staging_spl_dim.persp_clncl_dtl_descr) <> 'OR MINOR FLAT RATE' AND UPPER(intermediate_staging_spl_dim.persp_clncl_dtl_descr) IN ('OR MINOR 1 HR','OR MAJOR 1 HR','ROBOTIC OR TIME 1 HOUR') THEN 
   ROUND(intermediate_staging_chrg_fct_temp.quantity * intermediate_staging_spl_dim.spl_unit_cnvr * intermediate_staging_spl_dim.persp_clncl_dtl_unit,2) 
   ELSE 
   0  END as calculated_or_hrs
,row_number() over(partition by intermediate_staging_chrg_fct_temp.src_company_id, intermediate_staging_chrg_fct_temp.src_patient_id
Order by  intermediate_staging_chrg_fct_temp.service_date) as rec_num
FROM intermediate_staging_chrg_fct_temp
LEFT JOIN prmretlp.intermediate_staging_spl_dim on intermediate_staging_chrg_fct_temp.charge_code=intermediate_staging_spl_dim.cdm_cd and intermediate_staging_chrg_fct_temp.fcy_num=intermediate_staging_spl_dim.fcy_num
LEFT JOIN prmretlp.rev_cl_dim on intermediate_staging_chrg_fct_temp.revenue_code = rev_cl_dim.rev_cd 
LEFT JOIN prmretlp.hcpcs_dim  on intermediate_staging_chrg_fct_temp.cpt_code = hcpcs_dim.hcpcs_cd
LEFT JOIN prmretlp.dept_dim on  intermediate_staging_chrg_fct_temp.company_id = dept_dim.company_id and intermediate_staging_chrg_fct_temp.dept = dept_dim.department_code
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_chrg_cost_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_chrg_cost_fct AS 
(
select 
  src_company_id
, src_patient_id 
...
FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct
GROUP BY 1,2
);

--select 'Total in intermediate_staging_chrg_cost_fct' , count(*) from  pce_qe16_slp_prd_dm..intermediate_staging_chrg_cost_fct;
--intermediate_staging_encntr_pract_fct Table  creation based on Net 3 years Of patient Account Number


DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_pract_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_pract_fct AS 
(
  SELECT 
...
Order by  CH.service_start_date) as rec_num
  ...
  on Z.company_id = CH.company_id and Z.patient_id = CH.patient_id
  LEFT JOIN intermediate_staging_temp_physician_npi_spclty SPCL
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_specl_valid_ind IF EXISTS ; 
CREATE  TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_specl_valid_ind AS 
select distinct P.src_company_id, P.src_patient_id, 1 as specl_valid_ind
FROM intermediate_staging_encntr_pract_fct P 
INNER JOIN intermediate_staging_temp_eligible_encntr_data A 
on A.company_id = P.src_company_id and A.patient_id = P.src_patient_id and A.inpatient_outpatient_flag = 'I'
INNER JOIN intermediate_staging_temp_physician_npi_spclty S
...
--intermediate_staging_cpt_fct tABLE 
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_cpt_fct IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_cpt_fct AS 
SELECT 
hash8(Z.company_id) as src_company_id_hash
,hash8(Z.patient_id) as src_patient_id_hash
,Z.company_id as src_company_id
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_dgns_ccs_dim_cancer_only IF EXISTS;
CREATE TEMP TABLE intermediate_staging_temp_dgns_ccs_dim_cancer_only AS 
...
--intermediate_staging_encntr_dgns_fct Table  creation based on Net 3 years Of patient Account Number
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_dgns_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_dgns_fct AS 
SELECT 
		hash8(Z.company_id) as src_company_id_hash
	   ,hash8(Z.patient_id) as src_patient_id_hash
	   ,Z.company_id as src_company_id
...
Order by  DF.diagnosisseq) as rec_num 
  ...
  LEFT JOIN pce_qe16_slp_prd_dm..val_set_dim VSET_FCY
  ON VSET_FCY.cd = Z.company_id AND VSET_FCY.cohrt_id = 'FACILITY_CODES'
  LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_temp_dgns_ccs_dim_cancer_only CANCER
  on CANCER.dgns_cd = replace(DF.icd_code, '.','')
   DISTRIBUTE ON (src_company_id_hash, src_patient_id_hash);
   
-----intermediate_staging_encntr_pcd_fct Table  creation based on Net 3 years Of patient Account Number
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_pcd_fct IF EXISTS ; 
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_pcd_fct AS 
SELECT 
		hash8(Z.company_id) as src_company_id_hash
	   , hash8(Z.patient_id) as src_patient_id_hash
	   , Z.company_id as src_company_id
...
--       , DF.diagnosistype
  ...
  LEFT JOIN pce_qe16_slp_prd_dm..val_set_dim VSET_FCY
  ON VSET_FCY.cd = Z.company_id AND VSET_FCY.cohrt_id = 'FACILITY_CODES'
  LEFT JOIN intermediate_staging_temp_physician_npi_spclty SURGEON
  on SURGEON.company_id = DF.company_id and SURGEON.practitioner_code = DF.surgeon_code
    LEFT JOIN intermediate_staging_temp_physician_npi_spclty ORDERING
  on ORDERING.company_id = DF.company_id and ORDERING.practitioner_code = DF.orderingphysician
   DISTRIBUTE ON (src_company_id_hash, src_patient_id_hash);
------

--intermediate_staging_svc_ln_anl_fct Code
--based on cv_patbill / intermediate_staging_chrg_fct 

DROP TABLE intermediate_staging_temp_svc_based_on_patbill IF exists; 
CREATE TEMP TABLE intermediate_staging_temp_svc_based_on_patbill as 
select  
...
from intermediate_staging_temp_eligible_encntr_data EN
LEFT JOIN intermediate_staging_chrg_fct CH
on EN.company_id = CH.company_id and CH.patient_id = EN.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..svc_hier_dim_consolidated_new RNK
...
DROP TABLE intermediate_staging_temp_svc_based_on_pcdfct IF exists; 
CREATE TEMP TABLE intermediate_staging_temp_svc_based_on_pcdfct as 
select  
hash8(EN.company_id) as src_company_id_hash, hash8(EN.patient_id) as src_patient_id_hash,
...
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
from intermediate_staging_temp_eligible_encntr_data EN
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_encntr_pcd_fct X 
...
--?????
--Based on patcpt 
DROP TABLE intermediate_staging_temp_svc_based_on_patcpt IF exists; 
CREATE TEMP TABLE intermediate_staging_temp_svc_based_on_patcpt as 
select  
hash8(EN.company_id) as src_company_id_hash, hash8(EN.patient_id) as src_patient_id_hash,
EN.company_id, EN.patient_id ,EN.inpatient_outpatient_flag, 'cpt' as based_on,
--nvl(RNK.cd,'-100'),
...
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
from intermediate_staging_temp_eligible_encntr_data EN
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_cpt_fct X
on EN.company_id = X.company_id and EN.patient_id = X.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..svc_hier_dim_consolidated_new RNK
on lower(RNK.cd) = lower(X.cpt_code) and lower(RNK.cd_type) = 'cpt' ;

--MS-DRG (Inpatient)
DROP TABLE intermediate_staging_temp_svc_based_on_patdisch IF exists; 
CREATE TEMP TABLE intermediate_staging_temp_svc_based_on_patdisch as 
select  
hash8(EN.company_id) as src_company_id_hash, hash8(EN.patient_id) as src_patient_id_hash,
...
RNK.svc_cgy,RNK.svc_ln,RNK.sub_svc_ln , RNK.services as svc_nm, RNK.cd, RNK.cd_type,RNK.descr as cd_descr,
EN.admission_ts as adm_dt, EN.discharge_ts as dschrg_dt
from intermediate_staging_temp_eligible_encntr_data X
...
DROP TABLE intermediate_staging_svc_ln_anl_fct IF exists;
CREATE TABLE intermediate_staging_svc_ln_anl_fct As 
with  all_joined as (
select * from intermediate_staging_temp_svc_based_on_patbill union   -- //HCPCS / CPT 
select * from intermediate_staging_temp_svc_based_on_patdisch union  --Ms-Drg-code
select * from intermediate_staging_temp_svc_based_on_pcdfct  union   --ICD 9/10 PCS
select * from intermediate_staging_temp_svc_based_on_patcpt   --CPT 
 ),
all_joined_ms_drg_only AS 
(
  select 
         X.src_company_id_hash
       , X.src_patient_id_hash
       , X.company_id
       , X.patient_id
       , X.inpatient_outpatient_flag
       , X.based_on
       , X.svc_cgy
       , nvl(Z.mclaren_major_slp_grouping, X.svc_ln) as svc_ln
       , X.sub_svc_ln
       , X.svc_nm
       , X.cd
       , X.cd_type
       ...
       , X.dschrg_dt
from all_joined X
LEFT JOIN pce_qe16_slp_prd_dm..client_drg_svc_line_grouper_new Z 
ON Z.ms_drg_code = CAST(LPAD(CAST(coalesce(X.cd,'000') as INTEGER), 3,0 ) as Varchar(3)) 
WHERE lower(X.cd_type) = 'ms-drg'
),
all_joined_non_ms_drg_only AS 
(
  select *
  from all_joined X
WHERE X.cd_type NOT IN ('MS-DRG')
),
all_joined_null_only AS 
(
  select *
  from all_joined X
  WHERE X.cd_type is NULL 
),
combined_data AS 
(Select * FROM all_joined_ms_drg_only 
UNION 
Select * FROM all_joined_non_ms_drg_only 
UNION 
Select * FROM all_joined_null_only )
select
   X.*
...
DROP TABLE intermediate_staging_temp_encntr_svc_hier IF EXISTS;
CREATE TEMP TABLE intermediate_staging_temp_encntr_svc_hier AS 
With inpatient_svc as 
(
   select * from (
 --CODE CHANGE : AUG 2019 : INpatient MS_DRGDIM McLaren Major SLP GROUPING
--select Z.company_id , Z.patient_id , Z.inpatient_outpatient_flag , HIER.svc_cgy, HIER.svc_ln, HIER.sub_svc_ln, HIER.services, 
select Z.company_id , Z.patient_id , Z.inpatient_outpatient_flag , HIER.svc_cgy, nvl(SVCLN.svc_ln, HIER.svc_ln) as svc_ln, HIER.sub_svc_ln, HIER.services, 
HIER.svc_cgy_rnk as svc_cgy_rnk, 
HIER.svc_ln_rnk as svc_ln_rnk,
HIER.sub_svc_ln_rnk as sub_svc_ln_rnk,
HIER.svc_rnk as svc_rnk,
row_number() over(partition by Z.company_id, Z.patient_id ,Z.inpatient_outpatient_flag
Order by HIER.sub_svc_ln_rnk,HIER.svc_rnk) as rec_num
from intermediate_staging_temp_eligible_encntr_data Z
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_svc_ln_anl_fct SVCLN
on Z.company_id = SVCLN.company_id and Z.patient_id = SVCLN.patient_id and SVCLN.svc_ln is NOT NULL
LEFT JOIN pce_qe16_slp_prd_dm..svc_hier_dim_consolidated_new HIER
on HIER.cd = SVCLN.cd
WHERE Z.inpatient_outpatient_flag ='I' and upper(HIER.use_hry_ind) = 'YES' AND upper(HIER.ip_ind) = 'YES'
) A
WHERE A.rec_num =1
),
outpatient_svc as 
...
select * from (
 --CODE CHANGE : AUG 2019 : INpatient MS_DRGDIM McLaren Major SLP GROUPING
--select Z.company_id , Z.patient_id , Z.inpatient_outpatient_flag , HIER.svc_cgy, HIER.svc_ln, HIER.sub_svc_ln, HIER.services, 
select Z.company_id , Z.patient_id , Z.inpatient_outpatient_flag , HIER.svc_cgy,  HIER.svc_ln as svc_ln, HIER.sub_svc_ln, HIER.services, 
HIER.svc_cgy_rnk as svc_cgy_rnk, 
HIER.svc_ln_rnk as svc_ln_rnk,
HIER.sub_svc_ln_rnk as sub_svc_ln_rnk,
HIER.svc_rnk as svc_rnk,
row_number() over(partition by Z.company_id, Z.patient_id ,Z.inpatient_outpatient_flag
Order by  HIER.svc_ln_rnk,HIER.sub_svc_ln_rnk,HIER.svc_rnk) as rec_num
from intermediate_staging_temp_eligible_encntr_data Z
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_svc_ln_anl_fct SVCLN
on Z.company_id = SVCLN.company_id and Z.patient_id = SVCLN.patient_id and SVCLN.svc_ln is NOT NULL
LEFT JOIN pce_qe16_slp_prd_dm..svc_hier_dim_consolidated_new HIER
on HIER.cd = SVCLN.cd
WHERE Z.inpatient_outpatient_flag ='O'  and upper(HIER.use_hry_ind) = 'YES' AND upper(HIER.op_ind) = 'YES'
) A
WHERE A.rec_num =1)
...
--
DROP TABLE intermediate_staging_temp_fips_adr_dim IF EXISTS; 
CREATE TEMP TABLE intermediate_staging_temp_fips_adr_dim as 
(
   select 
...
DROP TABLE intermediate_staging_temp_ptnt_type_fcy_std_cd

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ptnt_type_fcy_std_cd AS (
...
DROP TABLE intermediate_staging_temp_ptnt_adm_dgns

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ptnt_adm_dgns AS (
		SELECT Z.company_id
		,Z.patient_id
		...
		FROM pce_qe16_slp_prd_dm..intermediate_staging_encntr_dgns_fct X
		INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON ENCNTR.company_id = X.company_id
...
DROP TABLE intermediate_staging_temp_ptnt_prim_dgns

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ptnt_prim_dgns AS (
		SELECT Z.company_id
		,Z.patient_id
...
DROP TABLE intermediate_staging_temp_ptnt_second_dgns

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ptnt_second_dgns AS (
		SELECT Z.company_id
		,Z.patient_id
...
SELECT count(*)
FROM intermediate_staging_temp_ptnt_second_dgns;--4,432,358

DROP TABLE intermediate_staging_temp_ptnt_trty_dgns

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ptnt_trty_dgns AS (
		SELECT Z.company_id
		,Z.patient_id
...
DROP TABLE intermediate_staging_temp_ptnt_prim_proc

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ptnt_prim_proc AS (
		SELECT Z.company_id
		,Z.patient_id
		...
		FROM pce_qe16_slp_prd_dm..intermediate_staging_encntr_pcd_fct X
		INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON ENCNTR.company_id = X.company_id
			AND ENCNTR.patient_id = X.patient_id
...
SELECT count(*)
FROM intermediate_staging_temp_ptnt_prim_proc;--332,451

DROP TABLE intermediate_staging_temp_ptnt_scdy_proc

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ptnt_scdy_proc AS (
		SELECT Z.company_id
		,Z.patient_id
...
SELECT count(*)
FROM intermediate_staging_temp_ptnt_scdy_proc;--145,743

DROP TABLE intermediate_staging_temp_ptnt_trty_proc

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ptnt_trty_proc AS (
		SELECT Z.company_id
		,Z.patient_id
...
DROP TABLE intermediate_staging_temp_obsrv

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_obsrv AS (
		SELECT pb.patient_id
		,pb.company_id
		,sum(pb.quantity) AS qty FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct pb WHERE pb.revenue_code = '0762' GROUP BY pb.patient_id
		,pb.company_id
		);--215561

--Code Change : Modified the existing logic (Rev Code) based on SPL Dimension

DROP TABLE intermediate_staging_temp_icu

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_icu AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
		,CASE WHEN(sum(ZOOM.quantity) =0 ) THEN NULL ELSE sum(ZOOM.quantity) END AS icu_days
		FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct ZOOM 
--		 INNER JOIN intermediate_staging_temp_dschrg_inpatient ON intermediate_staging_temp_dschrg_inpatient.patient_id = ZOOM.patient_id 
		 INNER JOIN pce_qe16_slp_prd_dm..intermediate_staging_spl_dim SP 
...
--Code Change : Modified the existing logic (Rev Code) based on SPL Dimension
DROP TABLE intermediate_staging_temp_ccu

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ccu AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
DROP TABLE intermediate_staging_temp_nrs

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_nrs AS (
		SELECT patient_id
		,company_id
		,count(DISTINCT pb.service_date) AS nrs_days FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct pb WHERE pb.revenue_code BETWEEN '0170'
			AND '0179' GROUP BY patient_id
		,company_id
		);--32134

DROP TABLE intermediate_staging_temp_rtne

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_rtne AS (
		SELECT patient_id
		,company_id
		,count(DISTINCT pb.service_date) AS rtne_days FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct pb WHERE (
			(
...
DROP TABLE intermediate_staging_temp_ed_case

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_ed_case AS (
		SELECT DISTINCT patient_id
		,company_id FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct PB INNER JOIN pce_qe16_slp_prd_dm..val_set_dim VSET_ED_CPT ON VSET_ED_CPT.cd = PB.cpt_code
		AND VSET_ED_CPT.cohrt_nm = 'ED_VISIT'
		);--2136885

DROP TABLE intermediate_staging_temp_dschrg_inpatient_ltcsnf

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_ltcsnf AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_ltcsnf_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
...
DROP TABLE intermediate_staging_temp_dschrg_inpatient_nbrn

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_nbrn AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_nbrn_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I'
		AND (
...
DROP TABLE intermediate_staging_temp_dschrg_inpatient_rehab

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_rehab AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_rehab_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I'
		AND (
...
DROP TABLE intermediate_staging_temp_dschrg_inpatient_psych

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_psych AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_psych_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I'
		AND (
...
DROP TABLE intermediate_staging_temp_dschrg_inpatient_spclcare

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_spclcare AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
...
--DROP TABLE intermediate_staging_temp_dschrg_inpatient_hospice
--
--IF EXISTS;
--	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_hospice AS (
--		SELECT DISTINCT Z.patient_id
--		,Z.company_id
--		,1 AS dschrg_hospice_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
...
DROP TABLE intermediate_staging_temp_payer_fcy_std_code

IF EXISTS;
--	CREATE TEMP TABLE intermediate_staging_temp_payer_fcy_std_code AS (
...
IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_hospice AS (
		...
		INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id
		INNER JOIN intermediate_staging_temp_payer_fcy_std_code VSET_HOSPICE
...
DROP TABLE intermediate_staging_temp_dschrg_inpatient_lipmip

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_lipmip AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
...
DROP TABLE intermediate_staging_temp_dschrg_inpatient_acute

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient_acute AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,CASE 
			WHEN (
				...
				THEN NULL
			ELSE 1
			END AS dschrg_acute_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id LEFT JOIN intermediate_staging_temp_dschrg_inpatient_nbrn NB ON NB.patient_id = Z.patient_id
		AND NB.company_id = Z.company_id LEFT JOIN intermediate_staging_temp_dschrg_inpatient_lipmip LIPMIP ON LIPMIP.patient_id = Z.patient_id
		AND LIPMIP.company_id = Z.company_id LEFT JOIN intermediate_staging_temp_dschrg_inpatient_rehab REHAB ON REHAB.patient_id = Z.patient_id
		AND REHAB.company_id = Z.company_id LEFT JOIN intermediate_staging_temp_dschrg_inpatient_psych PSYCH ON PSYCH.patient_id = Z.patient_id
		AND PSYCH.company_id = Z.company_id LEFT JOIN intermediate_staging_temp_dschrg_inpatient_ltcsnf LTCSNF ON LTCSNF.patient_id = Z.patient_id
		AND LTCSNF.company_id = Z.company_id LEFT JOIN intermediate_staging_temp_dschrg_inpatient_hospice HOSPICE ON HOSPICE.patient_id = Z.patient_id
		AND HOSPICE.company_id = Z.company_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I'
		AND Z.discharge_total_charges > 0
		);--323191 

DROP TABLE intermediate_staging_temp_dschrg_inpatient

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_dschrg_inpatient AS (
		SELECT DISTINCT Z.patient_id
		,Z.company_id
		,1 AS dschrg_ind FROM pce_qe16_oper_prd_zoom..cv_patdisch Z INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON Z.company_id = ENCNTR.company_id
		AND Z.patient_id = ENCNTR.patient_id WHERE Z.discharge_date IS NOT NULL
		AND Z.inpatient_outpatient_flag = 'I' GROUP BY 1
		,2
		);--323191

DROP TABLE intermediate_staging_temp_derived_ptnt_days

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_derived_ptnt_days AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
		,CASE WHEN (sum(ZOOM.quantity) =0) THEN NULL ELSE sum(ZOOM.quantity) END  AS ptnt_days 
		FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct ZOOM INNER JOIN intermediate_staging_temp_dschrg_inpatient ON intermediate_staging_temp_dschrg_inpatient.patient_id = ZOOM.patient_id WHERE (
			(
...
DROP TABLE intermediate_staging_temp_endoscopy_case

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_endoscopy_case AS (
		SELECT DISTINCT patient_id
		,company_id FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct WHERE revenue_code IN (
			'0750'
			) GROUP BY patient_id
		,company_id
		);
...
DROP TABLE intermediate_staging_temp_srgl_case

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_srgl_case AS (
		SELECT DISTINCT patient_id
		,company_id FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct CF 
		INNER JOIN pce_qe16_slp_prd_dm..intermediate_staging_spl_dim SP
...
--code change : Added logic to calculate  Lithotripsy  Cases based on SPL Dimension
   DROP TABLE intermediate_staging_temp_lithotripsy_case IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_lithotripsy_case AS (
		SELECT DISTINCT patient_id
		,company_id FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct CF 
		INNER JOIN pce_qe16_slp_prd_dm..intermediate_staging_spl_dim SP
		on CF.charge_code = SP.cdm_cd and CF.fcy_num = SP.fcy_num
 ...
 --code change : Added logic to calculate  CathLab Cases based on intermediate_staging_svc_ln_anl_fct
    DROP TABLE intermediate_staging_temp_cathlab_case IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_cathlab_case AS
	(
	  SELECT DISTINCT patient_id, company_id 
	  FROM pce_qe16_slp_prd_dm..intermediate_staging_svc_ln_anl_fct 
...
DROP TABLE intermediate_staging_temp_derived_ptnt_days_stepdown

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_derived_ptnt_days_stepdown AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
DROP TABLE intermediate_staging_temp_derived_ptnt_days_nbrn

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_derived_ptnt_days_nbrn AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
DROP TABLE intermediate_staging_temp_derived_ptnt_days_psych

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_derived_ptnt_days_psych AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
--CODE change: Modified the existing logic (REv Code) based on SPL Dimension
DROP TABLE intermediate_staging_temp_derived_ptnt_days_rehab

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_derived_ptnt_days_rehab AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
DROP TABLE intermediate_staging_temp_derived_ptnt_days_acute IF EXISTS; 


--Code Change: Modified the logic to calculate ptnt_days_acute
DROP TABLE intermediate_staging_temp_derived_ptnt_days_acute

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_derived_ptnt_days_acute AS (
		SELECT ZOOM.patient_id
		,ZOOM.company_id
...
--		 FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct ZOOM 
--		INNER JOIN intermediate_staging_temp_dschrg_inpatient ON intermediate_staging_temp_dschrg_inpatient.patient_id = ZOOM.patient_id WHERE (
...
--New
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_cnslt_pract_fct IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_cnslt_pract_fct as 
...
FROM intermediate_staging_temp_eligible_encntr_data Z
INNER JOIN pce_qe16_slp_prd_dm..intermediate_staging_encntr_pract_fct C1
...
FROM intermediate_staging_temp_eligible_encntr_data T1
...
DROP TABLE intermediate_staging_temp_surgeon_pract IF exists;
CREATE TEMP TABLE intermediate_staging_temp_surgeon_pract AS 
(
  ...
  from intermediate_staging_temp_eligible_encntr_data Z
  LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_encntr_pcd_fct P 
  on P.company_id = Z.company_id and Z.patient_id = P.patient_id 
  LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_temp_physician_npi_spclty PHY
...
DROP TABLE intermediate_staging_temp_physician_fcy_std_spclty

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_physician_fcy_std_spclty AS (
		SELECT DISTINCT PT.company_id
		,PT.practitioner_code
		,PT.practitioner_name
...
DROP TABLE intermediate_staging_temp_discharge_fcy_std_status_code

IF EXISTS;
	CREATE TEMP TABLE intermediate_staging_temp_discharge_fcy_std_status_code AS (
		SELECT DISTINCT ZOOM.discharge_status
		,DISSTATUS.dschrg_sts_cd
		,DISSTATUS.dschrg_sts_descr FROM pce_qe16_oper_prd_zoom..cv_patdisch ZOOM INNER JOIN intermediate_staging_temp_eligible_encntr_data ENCNTR ON ZOOM.company_id = ENCNTR.company_id
...
----Qualifiers 
--DROP TABLE intermediate_staging_temp_eligible_encntr_data IF EXISTS;
--	CREATE TEMP TABLE intermediate_staging_temp_eligible_encntr_data AS (
--		SELECT DISTINCT ZOOM.company_id
--		,ZOOM.patient_id , ZOOM.inpatient_outpatient_flag ,
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_table_all_ip_rows IF Exists; 
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_table_all_ip_rows  As 
with all_ip_recs as 
(
  select 
...
from pce_qe16_oper_prd_zoom..cv_patdisch X 
 ...
 INNER JOIN intermediate_staging_temp_eligible_encntr_data Z
...
FROM intermediate_staging_temp_table_all_ip_rows Z
...
and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now())) 
--Code Change : 08/08 As per McLaren Requet adding patient_Type exclusion
and  upper(Z.ptnt_tp_Cd) NOT in ('LIP','MIP')
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30')
Group by 1,2,3,4) X;
...
--     ROUND(sum(tot_chrg_amt),2) as tot_chrg_amt
FROM intermediate_staging_temp_table_all_ip_rows Z
INNER JOIN intermediate_staging_temp_ms_drg_dim_hist MSDRG
...
FROM intermediate_staging_temp_table_all_ip_rows PAID
WHERE PAID.est_acct_paid_ind ='Y' and PAID.src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid')  
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now())) 
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30') --85,855 Records
...
from intermediate_staging_temp_table_all_ip_rows UNPD
LEFT JOIN intermediate_staging_temp_ms_drg_dim_hist DRGWGHT
...
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30') --4578 Unpaid Cases with a Drg
UNION
...
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now())) 
;
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30');  --26 Records Unpaid Cases without Drg
...
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now())) 
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30') --7,559 Records
-- Unpaid Cases with a Drg (i.e Payments would be calculated based on Historical DRG Weights Ratio)
UNION 
select UNPD.*, 
ROUND(DRGWGHT.drg_wght * PAID.drg_weighted_pmnt_per_case, 2)  as est_net_rev_amt
...
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now()))
;
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30');  --4 Records Unpaid Cases without Drg
...
FROM intermediate_staging_temp_table_all_ip_rows ALLCASES
...
--WHERE cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now())) 
;
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now()))  AND  DATE ('2018-09-30')
--AND fcy_nm != 'Lansing';
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_table_all_op_rows IF Exists; 
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_table_all_op_rows As 
with all_op_recs as 
(
  select 
  X.company_id as fcy_nm, 
...
--ROUND(sum(tot_pymt_amt ) ,2) as  tot_pymt_amt,
--ROUND(sum(tot_chrg_amt),2) as tot_chrg_amt
FROM intermediate_staging_temp_table_all_op_rows Z
WHERE est_acct_paid_ind ='Y' and src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid')  
--Code Change : 08/08 As per McLaren Requet adding patient_Type exclusion
and upper(Z.ptnt_tp_Cd) NOT in ('LIP','MIP')
--case(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now())) 
-- and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now())) 
...
select UNPD.*, 
ROUND(UNPD.tot_chrg_amt * PAID.pymt_ratio ,2) as est_net_rev_amt
FROM intermediate_staging_temp_table_all_op_rows UNPD
...
ROUND(PAID.tot_pymt_amt +  abs(PAID.acct_bal_amt) ,2 ) as est_net_rev_amt
FROM intermediate_staging_temp_table_all_op_rows PAID
WHERE PAID.est_acct_paid_ind ='Y' and PAID.src_prim_payor_grp1 in ('BlueCross','Medicare','Medicaid')  
--and cast(dschrg_dt AS DATE) BETWEEN (Select DATE(Fiscal_start) - Interval '1 year' from intermediate_staging_temp_fiscal_year_tbl) and (now()- Day(Now())) 
;
...
select ALLCASES.*, 
ROUND(ALLCASES.tot_chrg_amt * RATIO.pymt_to_chrg_ratio ,2) as est_net_rev_amt
FROM intermediate_staging_temp_table_all_op_rows ALLCASES
LEFT JOIN pce_qe16_slp_prd_dm..manual_pymt_chrg_ratio RATIO
on RATIO.payor_group_1  = ALLCASES.src_prim_payor_grp1 AND RATIO.company_id = ALLCASES.fcy_nm
WHERE ALLCASES.src_prim_payor_grp1 in ('Domestic')  
...
FROM intermediate_staging_temp_table_all_ip_rows T
INNER JOIN pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct C
...
select T.fcy_nm, T.encntr_num ,T.tot_pymt_amt,T.tot_chrg_amt, T.acct_bal_amt, sum(C.total_charge) as dept_or_revenue_total_charge_amt
,CASE WHEN sum(C.total_charge) > 0 THEN 'Y' ELSE 'N' END as prof_chrg_ind
FROM intermediate_staging_temp_table_all_op_rows T
INNER JOIN pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct C
on C.company_id = T.fcy_nm and T.encntr_num = C.patient_id
WHERE (
--Department Exclusion
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_net_rvu_fct_x IF EXISTS;
CREATE TEMP TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_net_rvu_fct_x AS
with combined as 
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_net_rvu_fct IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_net_rvu_fct AS
with combined as 
(select * from pce_qe16_slp_prd_dm..op_encntr_net_rvu
UNION 
select * from pce_qe16_slp_prd_dm..ip_encntr_net_rvu),
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_hist_pymt_ratio IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_hist_pymt_ratio as 
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_hist_pymt_ratio_drg_wghts IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_hist_pymt_ratio_drg_wghts as 
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_net_rvu_model IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_net_rvu_model as 
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_encntr_dgns_fct_with_cancer_case IF EXISTS;
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_encntr_dgns_fct_with_cancer_case AS 
...
row_number() over(partition by X.company_id, X.patient_id ORDER BY X.diagnosistype ) as rank_num
  FROM intermediate_staging_encntr_dgns_fct X
 ...
 DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_blood_util_qty IF EXISTS; 
CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_blood_util_qty AS 
SELECT X.src_company_id, X.src_patient_id,
SUM(X.quantity) AS blood_util_qty
FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct X
...
DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_lab_util_qty IF EXISTS;

CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_temp_lab_util_qty AS 
SELECT X.src_company_id, X.src_patient_id,
SUM(X.quantity) AS lab_util_qty
FROM pce_qe16_slp_prd_dm..intermediate_staging_chrg_fct X
WHERE X.department_group =  'Lab' AND X.cpt_code NOT IN ('82948','36415')
GROUP BY 1,2; 

 ---------------------------------------------------------------------------------------


DROP TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_anl_fct IF EXISTS;

	CREATE TABLE pce_qe16_slp_prd_dm..intermediate_staging_encntr_anl_fct AS
...
--   ,ACO_MSDRG.case_mix_idnx_num
  ...
  --,ZOOM.subfacility AS sub_fcy
	...
	,FCY_REF.bed_cnt
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient_nbrn.patient_id IS NULL
			THEN NULL
		ELSE intermediate_staging_temp_dschrg_inpatient_nbrn.dschrg_nbrn_ind
		END AS dschrg_nbrn_ind
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient_rehab.patient_id IS NULL
			THEN NULL
		ELSE intermediate_staging_temp_dschrg_inpatient_rehab.dschrg_rehab_ind
		END AS dschrg_rehab_ind
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient_psych.patient_id IS NULL
			THEN NULL
		ELSE intermediate_staging_temp_dschrg_inpatient_psych.dschrg_psych_ind
		END AS dschrg_psych_ind
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient_ltcsnf.patient_id IS NULL
			THEN NULL
		ELSE intermediate_staging_temp_dschrg_inpatient_ltcsnf.dschrg_ltcsnf_ind
		END AS dschrg_ltcsnf_ind
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient_hospice.patient_id IS NULL
			THEN NULL
		ELSE intermediate_staging_temp_dschrg_inpatient_hospice.dschrg_hospice_ind
		END AS dschrg_hospice_ind
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient_spclcare.patient_id IS NULL
			THEN NULL
		ELSE intermediate_staging_temp_dschrg_inpatient_spclcare.dschrg_spclcare_ind
		END AS dschrg_spclcare_ind
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient_lipmip.patient_id IS NULL
			THEN NULL
		ELSE intermediate_staging_temp_dschrg_inpatient_lipmip.dschrg_lipmip_ind
		END AS dschrg_lipmip_ind
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient_acute.patient_id IS NULL
			THEN NULL
		ELSE intermediate_staging_temp_dschrg_inpatient_acute.dschrg_acute_ind
		END AS dschrg_acute_ind
	,CASE 
		WHEN intermediate_staging_temp_dschrg_inpatient.patient_id IS NOT NULL
		...
		END AS dschrg_ind
	,CASE 
		WHEN intermediate_staging_temp_obsrv.qty > 0
			THEN intermediate_staging_temp_obsrv.qty
		ELSE intermediate_staging_temp_obsrv.qty
		END AS obsrv_hours
	,CASE 
		WHEN obsrv_hours > 0
			THEN obsrv_hours / 24
	...
	, ( nvl(intermediate_staging_temp_derived_ptnt_days_acute.ptnt_days_acute ,0)+ 
	   nvl(intermediate_staging_temp_ccu.ccu_days, 0) + 
	   nvl(intermediate_staging_temp_icu.icu_days, 0) + 
	   nvl(intermediate_staging_temp_derived_ptnt_days_stepdown.ptnt_days_stepdown, 0) +
	   nvl(intermediate_staging_temp_derived_ptnt_days_nbrn.ptnt_days_nbrn, 0) + 
	   nvl(intermediate_staging_temp_derived_ptnt_days_rehab.ptnt_days_rehab, 0) +
	   nvl(intermediate_staging_temp_derived_ptnt_days_psych.ptnt_days_psych, 0)) AS ptnt_days
	,CASE 
		WHEN intermediate_staging_temp_derived_ptnt_days_psych.patient_id IS NOT NULL
			THEN intermediate_staging_temp_derived_ptnt_days_psych.ptnt_days_psych
		ELSE NULL
		END AS ptnt_days_pysch
	,CASE 
		WHEN intermediate_staging_temp_derived_ptnt_days_rehab.patient_id IS NOT NULL
			THEN intermediate_staging_temp_derived_ptnt_days_rehab.ptnt_days_rehab
		ELSE NULL
		END AS ptnt_days_rehab
	,CASE 
		WHEN intermediate_staging_temp_derived_ptnt_days_nbrn.patient_id IS NOT NULL
			THEN intermediate_staging_temp_derived_ptnt_days_nbrn.ptnt_days_nbrn
		ELSE NULL
		END AS ptnt_days_nbrn
	,CASE 
		WHEN intermediate_staging_temp_derived_ptnt_days_stepdown.patient_id IS NOT NULL
			THEN intermediate_staging_temp_derived_ptnt_days_stepdown.ptnt_days_stepdown
		ELSE NULL
		END AS ptnt_days_stepdown
	,CASE 
		WHEN intermediate_staging_temp_derived_ptnt_days_acute.patient_id IS NOT NULL
			THEN intermediate_staging_temp_derived_ptnt_days_acute.ptnt_days_acute
		ELSE NULL
		END AS ptnt_days_acute
	,CASE 
		WHEN intermediate_staging_temp_icu.patient_id IS NOT NULL
			THEN intermediate_staging_temp_icu.icu_days
		ELSE NULL
		END AS icu_days
	,CASE 
		WHEN intermediate_staging_temp_ccu.patient_id IS NOT NULL
			THEN intermediate_staging_temp_ccu.ccu_days
		ELSE NULL
		END AS ccu_days
	,CASE 
		WHEN intermediate_staging_temp_nrs.patient_id IS NOT NULL
			THEN intermediate_staging_temp_nrs.nrs_days
		ELSE NULL
		END AS nrs_days
	,CASE 
		WHEN intermediate_staging_temp_rtne.patient_id IS NOT NULL
			THEN intermediate_staging_temp_rtne.rtne_days
		ELSE NULL
		END AS rtne_days
	,CASE 
		WHEN intermediate_staging_temp_ed_case.patient_id IS NOT NULL
			THEN 1
		ELSE NULL
		END AS ed_case_ind
	...
	,CASE 
		WHEN intermediate_staging_temp_endoscopy_case.patient_id IS NULL
			THEN NULL
		ELSE 1
		END AS endoscopy_case_ind
	,CASE 
		WHEN intermediate_staging_temp_srgl_case.patient_id IS NULL
			THEN NULL
		ELSE 1
		END AS srgl_case_ind
	,CASE 
		WHEN intermediate_staging_temp_lithotripsy_case.patient_id IS NULL
			THEN NULL
		ELSE 1
		END AS lithotripsy_case_ind
	,CASE 
		WHEN intermediate_staging_temp_cathlab_case.patient_id IS NULL
			THEN NULL
		ELSE 1
...
--Code Change : Zoom gets from Encntr but Integrated Mart gets from intermediate_staging_encntr_dgns_fct so commenting Integrated Version
...
--Code Change: Zoom gets from Encntr but Integrated Mart gets from intermediate_staging_encntr_pcd_fct so commenting Integrated Version
...
FROM pce_qe16_oper_prd_zoom..cv_patdisch ZOOM
INNER JOIN pce_qe16_slp_prd_dm..intermediate_staging_temp_eligible_encntr_data ZOOM3YRS
on ZOOM.company_id = ZOOM3YRS.company_id and ZOOM.patient_id = ZOOM3YRS.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_chrg_cost_fct CHRGRCC
on ZOOM.company_id = CHRGRCC.src_company_id and ZOOM.patient_id = CHRGRCC.src_patient_id
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_encntr_qly_anl_fct QADV ON ZOOM.patient_id = QADV.encntr_num --AND ZOOM.company_id = QADV.fcy_num 
...
LEFT JOIN intermediate_staging_temp_dschrg_inpatient ON intermediate_staging_temp_dschrg_inpatient.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_physician_npi_spclty ATTENDPRACTSPEC ON ZOOM.company_id = ATTENDPRACTSPEC.company_id
	AND ZOOM.attending_practitioner_code = ATTENDPRACTSPEC.practitioner_code
LEFT JOIN intermediate_staging_temp_physician_npi_spclty ADMITPRACTSPEC ON ZOOM.company_id = ADMITPRACTSPEC.company_id
	AND ZOOM.admitting_practitioner_code = ADMITPRACTSPEC.practitioner_code
LEFT JOIN intermediate_staging_temp_dschrg_inpatient_hospice ON intermediate_staging_temp_dschrg_inpatient_hospice.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient_hospice.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_obsrv ON intermediate_staging_temp_obsrv.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_obsrv.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_icu ON intermediate_staging_temp_icu.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_icu.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_ccu ON intermediate_staging_temp_ccu.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_ccu.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_nrs ON intermediate_staging_temp_nrs.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_nrs.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_rtne ON intermediate_staging_temp_rtne.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_rtne.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_ed_case ON intermediate_staging_temp_ed_case.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_ed_case.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_dschrg_inpatient_nbrn ON intermediate_staging_temp_dschrg_inpatient_nbrn.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient_nbrn.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_dschrg_inpatient_rehab ON intermediate_staging_temp_dschrg_inpatient_rehab.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient_rehab.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_dschrg_inpatient_psych ON intermediate_staging_temp_dschrg_inpatient_psych.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient_psych.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_dschrg_inpatient_ltcsnf ON intermediate_staging_temp_dschrg_inpatient_ltcsnf.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient_ltcsnf.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_dschrg_inpatient_spclcare ON intermediate_staging_temp_dschrg_inpatient_spclcare.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient_spclcare.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_dschrg_inpatient_lipmip ON intermediate_staging_temp_dschrg_inpatient_lipmip.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient_lipmip.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_dschrg_inpatient_acute ON intermediate_staging_temp_dschrg_inpatient_acute.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_dschrg_inpatient_acute.company_id = ZOOM.company_id
--Code Change: Commenting the following since ptnt_days_total would be based on SPL dimension
--LEFT JOIN intermediate_staging_temp_derived_ptnt_days ON intermediate_staging_temp_derived_ptnt_days.patient_id = ZOOM.patient_id
--AND intermediate_staging_temp_derived_ptnt_days.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_srgl_case ON intermediate_staging_temp_srgl_case.patient_id = ZOOM.patient_id 
	AND intermediate_staging_temp_srgl_case.company_id = ZOOM.company_id
--Code Change : To add LITHOTRIPSY, Endoscopy and Cath Lab Case INDICATOR
LEFT JOIN intermediate_staging_temp_cathlab_case ON intermediate_staging_temp_cathlab_case.patient_id = ZOOM.patient_id 
	AND intermediate_staging_temp_cathlab_case.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_lithotripsy_case ON intermediate_staging_temp_lithotripsy_case.patient_id = ZOOM.patient_id 
	AND intermediate_staging_temp_lithotripsy_case.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_endoscopy_case ON intermediate_staging_temp_endoscopy_case.patient_id = ZOOM.patient_id 
	AND intermediate_staging_temp_endoscopy_case.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_derived_ptnt_days_nbrn ON intermediate_staging_temp_derived_ptnt_days_nbrn.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_derived_ptnt_days_nbrn.company_id = ZOOM.company_id
--Code Change : To add Ptnt_Days_stepdown
LEFT JOIN intermediate_staging_temp_derived_ptnt_days_stepdown ON intermediate_staging_temp_derived_ptnt_days_stepdown.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_derived_ptnt_days_stepdown.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_derived_ptnt_days_rehab ON intermediate_staging_temp_derived_ptnt_days_rehab.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_derived_ptnt_days_rehab.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_derived_ptnt_days_acute ON intermediate_staging_temp_derived_ptnt_days_acute.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_derived_ptnt_days_acute.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_derived_ptnt_days_psych ON intermediate_staging_temp_derived_ptnt_days_psych.patient_id = ZOOM.patient_id
	AND intermediate_staging_temp_derived_ptnt_days_psych.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_payer_fcy_std_code PRIMPAYER ON PRIMPAYER.company_id = ZOOM.company_id
	AND PRIMPAYER.fcy_payer_code = ZOOM.primary_payer_code
LEFT JOIN intermediate_staging_temp_payer_fcy_std_code SECONPAYER ON SECONPAYER.company_id = ZOOM.company_id
	AND SECONPAYER.fcy_payer_code = ZOOM.secondary_payer_code
LEFT JOIN intermediate_staging_temp_payer_fcy_std_code TRTYPAYER ON TRTYPAYER.company_id = ZOOM.company_id
	AND TRTYPAYER.fcy_payer_code = ZOOM.tertiarypayorplan
LEFT JOIN intermediate_staging_temp_payer_fcy_std_code QTRPAYER ON QTRPAYER.company_id = ZOOM.company_id
...
LEFT JOIN intermediate_staging_temp_discharge_fcy_std_status_code DISSTATUS ON DISSTATUS.discharge_status = ZOOM.discharge_status
LEFT JOIN pce_qe16_slp_prd_dm..stnd_ptnt_zip_dim ZIPCODE ON ZIPCODE.ptnt_zip_cd = substr(ZOOM.residential_zip_code, 1, 5)
LEFT JOIN intermediate_staging_temp_ptnt_type_fcy_std_cd PATTYPE ON PATTYPE.patient_type_code = ZOOM.patient_type
	AND PATTYPE.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_ptnt_prim_dgns PRIMDGNS ON PRIMDGNS.patient_id = ZOOM.patient_id
	AND PRIMDGNS.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_ptnt_second_dgns SCDYDGNS ON SCDYDGNS.patient_id = ZOOM.patient_id
	AND SCDYDGNS.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_ptnt_trty_dgns TRTYDGNS ON TRTYDGNS.patient_id = ZOOM.patient_id
	AND TRTYDGNS.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_ptnt_prim_proc PRIMPROC ON PRIMPROC.patient_id = ZOOM.patient_id
	AND PRIMPROC.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_ptnt_scdy_proc SCDYPROC ON SCDYPROC.patient_id = ZOOM.patient_id
	AND SCDYPROC.company_id = ZOOM.company_id
LEFT JOIN intermediate_staging_temp_ptnt_trty_proc TRTYPROC ON TRTYPROC.patient_id = ZOOM.patient_id
...
LEFT JOIN intermediate_staging_temp_encntr_svc_hier SVCRNK
on SVCRNK.company_id = ZOOM.company_id and SVCRNK.patient_id = ZOOM.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_temp_surgeon_pract SURGEON
on SURGEON.company_id = ZOOM.company_id and SURGEON.patient_id = ZOOM.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_encntr_cnslt_pract_fct CNSLT
on CNSLT.src_company_id = ZOOM.company_id and CNSLT.src_patient_id = ZOOM.patient_id
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_encntr_net_rvu_fct NETREV
...
LEFT JOIN  pce_qe16_slp_prd_dm..intermediate_staging_temp_specl_valid_ind SPECLVALID
on SPECLVALID.src_company_id = ZOOM.company_id and ZOOM.patient_id = SPECLVALID.src_patient_id 
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_temp_encntr_dgns_fct_with_cancer_case CANCER
on CANCER.src_patient_id = ZOOM.patient_id AND CANCER.src_company_id = ZOOM.company_id
--CODE CHANGE : AUG 2019 Blood and Lab Utilization
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_temp_blood_util_qty BLOOD
on BLOOD.src_patient_id = ZOOM.patient_id AND BLOOD.src_company_id = ZOOM.company_id
LEFT JOIN pce_qe16_slp_prd_dm..intermediate_staging_temp_lab_util_qty LAB
on LAB.src_patient_id = ZOOM.patient_id AND LAB.src_company_id = ZOOM.company_id
---------
