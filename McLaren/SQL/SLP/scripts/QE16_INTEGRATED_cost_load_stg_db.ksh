...
v_crt_stm_2='1 as cnt from intermediate_encntr_cst_fct group by fcy_nm, encntr_num distribute on (fcy_nm, encntr_num);'
...
nzsql -h ${v_nzHost} -u ${v_nzUsr} -d pce_qe16_slp_${env}_dm -r -t -f /ds_data1/${v_project}/sql/QE16/SLP/QE16_INTEGRATED_att_gen.sql >> /ds_data1/${v_project}/sql/QE16/SLP/encntr_cost_anl_fct_stg_db.sql
        if [ $? -ne 0 ]; then
...
nzsql -h ${v_nzHost} -u ${v_nzUsr} -d pce_qe16_slp_${env}_dm -r -t -f /ds_data1/${v_project}/sql/QE16/SLP/encntr_cost_anl_fct_stg_db.sql
