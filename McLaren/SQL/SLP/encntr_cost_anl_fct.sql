...
create table stage_encntr_cst_anl_fct as select fcy_nm, encntr_num,
 ...
 max(case when client_revenue_code_group='Sub-acute Care' then ttl_cst_amt end ) as sbct_cr_ttl_cst_amt,
 max(case when client_revenue_code_group='Cardiology' then indrct_cst_amt end ) as crdlgy_indrct_cst_amt,
 max(case when client_revenue_code_group='Sub-acute Care' then indrct_cst_amt end ) as sbct_cr_indrct_cst_amt,
 ...
 max(case when client_revenue_code_group='Blood & Blood Products' then indrct_cst_amt end ) as bld__bld_prdcts_indrct_cst_amt,
 ...
 max(case when client_revenue_code_group='Observation' then indrct_cst_amt end ) as obsrvtn_indrct_cst_amt,
 max(case when client_revenue_code_group='Blood & Blood Products' then ttl_cst_amt end ) as bld__bld_prdcts_ttl_cst_amt,
 ...
 max(case when client_revenue_code_group='Blood & Blood Products' then ttl_chrg_amt end ) as bld__bld_prdcts_ttl_chrg_amt,
 max(case when client_revenue_code_group='Emergency Room' then ttl_chrg_amt end ) as emrgncy_rm_ttl_chrg_amt,
 max(case when client_revenue_code_group='Blood & Blood Products' then drct_cst_amt end ) as bld__bld_prdcts_drct_cst_amt,
 max(case when client_revenue_code_group='Critical Care' then ttl_cst_amt end ) as crtcl_cr_ttl_cst_amt,
 max(case when client_revenue_code_group='Observation' then drct_cst_amt end ) as obsrvtn_drct_cst_amt,
 max(case when client_revenue_code_group='Observation' then ttl_cst_amt end ) as obsrvtn_ttl_cst_amt,
 ...
 max(case when client_revenue_code_group='Observation' then ttl_chrg_amt end ) as obsrvtn_ttl_chrg_amt,
 ...
 max(case when client_revenue_code_group='Sub-acute Care' then drct_cst_amt end ) as sbct_cr_drct_cst_amt,
 ...
 max(case when client_revenue_code_group='Sub-acute Care' then ttl_chrg_amt end ) as sbct_cr_ttl_chrg_amt,
...
1 as cnt, hash8(fcy_nm || cast(chr(45) as varchar(1)) || encntr_num) as fcy_encntr_hashkey from pce_qe16_slp_prd_dm..intermediate_encntr_cst_fct group by fcy_nm, encntr_num distribute on (fcy_nm, encntr_num,fcy_encntr_hashkey); 

drop table prd_encntr_cst_anl_fct_prev if exists; 
alter table prd_encntr_cst_anl_fct rename to prd_encntr_cst_anl_fct_prev; 
