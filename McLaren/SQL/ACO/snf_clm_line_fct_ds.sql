...
DROP TABLE snf_clm_line_fct_ds IF EXISTS;
...
select fcy_case_id as clm_id, snf_ed_vst_ind, days_fm_snf_to_ed_vst from dmart_snf_ed_visits;
...
CREATE TABLE snf_clm_line_fct_ds AS
  SELECT
    cf.clm_id,
    cf.clm_line_num,
    cf.ctr_id,
    ...
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.ed_care_needed_not_prvntable_pct end as ed_care_needed_not_prvntable_pct,
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.ed_care_needed_prvntable_avoidable_pct end as ed_care_needed_prvntable_avoidable_pct,
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.treatable_emrgnt_ptnt_care_pct end as treatable_emrgnt_ptnt_care_pct,
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.non_emrgnt_rel_pct end as non_emrgnt_rel_pct,
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.alc_rel_pct end as alc_rel_pct,
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.drug_rel_pct end as drug_rel_pct,
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.injry_rel_pct end as injry_rel_pct,
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.psychology_rel_pct end as psychology_rel_pct,
    case when cmd.care_svc_cgy_nm='Emergency Room' then nyu.unclsfd_pct end as unclsfd_pct,
...
--Added SNF Discharge based on algorithm defined on SNF Discharge Fact
    ...
    sdnew.snf_unld_ind,
    CASE WHEN cmd.cst_modl_line_cgy_nm = 'SNF' and cf.clm_line_num='001' AND dsd.dschrg_sts_cd <> '30' 
	THEN 1
    WHEN cmd.cst_modl_line_cgy_nm = 'SNF' and cf.clm_line_num='001' AND dsd.dschrg_sts_cd <> '30' and cf.paid_amt < 0
	THEN -1
    ELSE NULL END AS snf_w_zero_unld_ind,
 --Added logic for Negative Adjustments   
    ...
    CASE WHEN cmd.care_svc_sub_cgy_nm='Urgent Care' THEN cf.cst_modl_utlz_cnt ELSE 0 END as urgnt_ind, 
    hcp.anul_wlns_vst_ind,
    CASE WHEN ed_vst_ind = 1 OR in_ptnt_adm_ind = 1
      THEN 1 END                                                            AS hsp_vst_ind,
  ...
  FROM
    ...
    LEFT OUTER JOIN snf_ed_vst_dm sed on cf.clm_id=sed.clm_id and cf.clm_line_num = 1
    LEFT OUTER JOIN snf_dschrg_fct sd on cf.clm_id=sd.clm_id and cf.clm_line_num=sd.clm_line_num
    LEFT OUTER JOIN snf_dschrg_fct_new sdnew on cf.fcy_case_id=sdnew.fcy_case_id and cf.clm_id=sdnew.last_clm_id and cf.clm_line_num = 1
    ...
    LEFT OUTER JOIN post_dschrg_vst_fct pdvf on cf.clm_id = pdvf.encntr_num and cf.clm_line_num = 1
