...
do
	...
	nzsql -h ${v_nzHost} -u ${v_nzUsr} -d pce_qe16_aco_${env}_dm -c "
		insert into hist_bene_pcp_attr
			select distinct pln_mbr_sk, mbr_id_num, bill_pvdr_sk, pcs_cnt, paid_amt, last_date,strt_mn, end_mn,npi,actv_sts, rnk,
                1 in_ntw_ind, 'MHPN Physician' as attr from temp_hist_bene_pcp_services
        where rnk=$v_cnt and mhpn_indicator=1 and actv_sts=1 and pln_mbr_sk not in (select pln_mbr_sk from hist_bene_pcp_attr);"
...
do
	...
	nzsql -h ${v_nzHost} -u ${v_nzUsr} -d pce_qe16_aco_${env}_dm -c "
        ...
        where rnk=$v_cnt and mpp_indicator=1 and actv_sts=1 and pln_mbr_sk not in (select pln_mbr_sk from hist_bene_pcp_attr);"
...
##Assignment of Inactive/Retired Physicians
	nzsql -h ${v_nzHost} -u ${v_nzUsr} -d pce_qe16_aco_${env}_dm -c "
		...
		from temp_hist_bene_pcp_services 
			where rnk=1 and mhpn_indicator=1 and  actv_sts=0 and pln_mbr_sk not in (select pln_mbr_sk from hist_bene_pcp_attr);"
        nzsql -h ${v_nzHost} -u ${v_nzUsr} -d pce_qe16_aco_${env}_dm -c "
                insert into hist_bene_pcp_attr
                        select distinct pln_mbr_sk, mbr_id_num, bill_pvdr_sk, pcs_cnt, paid_amt, last_date,strt_mn, end_mn,npi,actv_sts, rnk,
                1 as in_ntw_ind, 'Retired/Inactive Physician' as attr
                from temp_hist_bene_pcp_services
                        where rnk=1 and mpp_indicator=1 and  actv_sts=0 and pln_mbr_sk not in (select pln_mbr_sk from hist_bene_pcp_attr);"
...
##Remove if duplicates are found
	nzsql -h ${v_nzHost} -u ${v_nzUsr} -d pce_qe16_aco_${env}_dm -c "delete from hist_bene_pcp_attr where rowid in (select min(rowid) from hist_bene_pcp_attr group by mbr_id_num having count(1)>1);"
